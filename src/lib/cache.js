...
// Use UnifiedCache when Redis is available for horizontal scaling
let cache;

if (process.env.REDIS_URL) {
  // Use Redis-backed cache for production/distributed environments
  const { UnifiedCache } = require('./redisCache');
  cache = new UnifiedCache({
    prefix: 'app:data:',
    maxSize: 2000,
    defaultTTL: TTL.METRICS,
  });
  console.log('[Cache] Using UnifiedCache (Redis-backed)');
} else {
  ...
  console.log('[Cache] Using MemoryCache (in-memory only)');
}
