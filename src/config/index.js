...
// In production (Railway), DATA_DIR should point to persistent storage
const DATA_DIR = process.env.DATA_DIR || path.join(__dirname, '../../data');

// Check if we're on ephemeral filesystem (Railway, Heroku, etc.)
...
const config = {
  ...
  storage: {
    ...
    // Use database for state instead of files when on ephemeral storage
    useDbForState: isEphemeralStorage || process.env.USE_DB_FOR_STATE === 'true',
  },

  // Database
  database: {
    url: process.env.DATABASE_URL,
    path: process.env.DATABASE_PATH || path.join(DATA_DIR, 'stocks.db'),
  ...
  ai: {
    ...
    // UPDATED: Reduced monthly budget from $100 to $50 per production requirements
    monthlyBudget: parseFloat(process.env.LLM_MONTHLY_BUDGET) || 50,
...
/**
 * Get absolute path within the data directory
 ...
 * @returns {boolean}
 */
function dataFileExists(...segments) {
  return fs.existsSync(getDataPath(...segments));
}

/**
 * Ensure the data directory exists
 * @param {...string} segments - Subdirectory path segments
 */
function ensureDataDir(...segments) {
  const dir = segments.length > 0 ? getDataPath(...segments) : DATA_DIR;
...
/**
 * Check if we should use database for state storage
 * Returns true if on ephemeral filesystem or explicitly configured
 */
function shouldUseDbForState() {
  return config.storage.useDbForState;
}
