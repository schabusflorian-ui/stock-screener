 ...
 * Requires: Ultra tier (backtesting feature)
 */
router.post('/walk-forward', requireAuth, requireFeature('backtesting'), checkUsageLimit('backtest_runs_monthly'), async (req, res) => {
  try {
    const {
      portfolioId,
      strategyName,
 ...
 * Requires: Ultra tier (backtesting feature)
 */
router.post('/cpcv', requireAuth, requireFeature('backtesting'), checkUsageLimit('backtest_runs_monthly'), async (req, res) => {
  try {
...
// ============================================
// Factor Lab Endpoints
// ============================================

const { FactorBacktestEngine } = require('../../services/backtesting/factorBacktestEngine');
const { ScreeningBacktestEngine, SCREEN_PRESETS } = require('../../services/backtesting/screeningBacktestEngine');
const { FactorSignalGenerator } = require('../../services/factors/factorSignalGenerator');

/**
 * POST /api/backtesting/factor-combination
 * Backtest a factor combination strategy
 */
router.post('/factor-combination', async (req, res) => {
  try {
    const {
      factorWeights,
      startDate,
    ...
    if (!factorWeights) {
      return res.status(400).json({
        success: false,
        error: 'factorWeights is required (e.g., { value: 0.4, quality: 0.3, momentum: 0.3 })'
      });
    }
    ...
    console.error('Factor combination backtest error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * POST /api/backtesting/screen-backtest
 * Backtest custom screening criteria
 */
router.post('/screen-backtest', async (req, res) => {
  try {
    const {
      criteria,
      startDate,
    ...
    if (!criteria) {
      return res.status(400).json({
        success: false,
        error: 'criteria is required (e.g., { minROIC: 15, maxDebtToEquity: 0.5 })'
      });
    }
    ...
    console.error('Screen backtest error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * POST /api/backtesting/preset-screen-backtest
 * Backtest a preset screening strategy
 */
router.post('/preset-screen-backtest', async (req, res) => {
  try {
    const {
      preset,
      startDate,
    ...
    if (!preset) {
      return res.status(400).json({
        success: false,
        error: `preset is required. Available: ${Object.keys(SCREEN_PRESETS).join(', ')}`
      });
    }
    ...
    console.error('Preset screen backtest error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * GET /api/backtesting/screen-presets
 * Get available screening presets
 */
router.get('/screen-presets', (req, res) => {
  try {
    const presets = ScreeningBacktestEngine.getPresets();
...
/**
 * POST /api/backtesting/factor-signals
 * Generate current factor-based trading signals
 */
router.post('/factor-signals', async (req, res) => {
  try {
    const {
      factorWeights,
      topN = 10,
    ...
    if (!factorWeights) {
      return res.status(400).json({
        success: false,
        error: 'factorWeights is required'
      });
    }
    ...
    console.error('Factor signals error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * GET /api/backtesting/factor-signal/:symbol
 * Get factor signal for a specific stock
 */
router.get('/factor-signal/:symbol', async (req, res) => {
  try {
    const { symbol } = req.params;
    const factorWeights = {
      value: parseFloat(req.query.value) || 0.2,
      quality: parseFloat(req.query.quality) || 0.2,
      momentum: parseFloat(req.query.momentum) || 0.2,
      growth: parseFloat(req.query.growth) || 0.2,
      size: parseFloat(req.query.size) || 0.1,
