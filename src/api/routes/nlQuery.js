...
function getMockResponse(query) {
  ...
  const confirmation = buildConfirmation(intent, potentialSymbols, companyResolutions);
 ...
 * Uses AI query quota (free tier has 10/month limit)
 */
 ...
 * Uses AI query quota (free tier has 10/month limit)
 */
router.post('/query/stream', optionalAuth, attachSubscription, checkUsageLimit('ai_queries_monthly'), async (req, res) => {
  // Set up SSE headers
 ...
 * Used by the Screening page for direct NL → filter conversion
 */
router.post('/screen', asyncHandler(async (req, res) => {
  ...
  console.log(`[NL Screen] Processing: "${query}"`);

  const llmHandler = getLLMHandler();

  try {
    const result = await llmHandler.extractScreeningCriteria(query, context || {});

    // Log result for debugging
    console.log(`[NL Screen] Result: intent=${result.intent}, confidence=${result.confidence}, source=${result.source}`);

    return res.json(result);
  } catch (error) {
    console.error('[NL Screen] Error:', error);
    return sendError(res, ERROR_CODES.INTERNAL_ERROR, 'Failed to extract screening criteria', safeErrorMessage(error));
  }
}));
