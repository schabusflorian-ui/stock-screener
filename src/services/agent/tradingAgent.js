...
// NEW: Tax-aware trading support
let PortfolioTaxService = null;
let getTaxRegime = null;
try {
  PortfolioTaxService = require('../portfolio/portfolioTaxService');
  const taxRegimes = require('../costs/taxRegimes');
  getTaxRegime = taxRegimes.getTaxRegime;
} catch (e) {
  console.warn('Tax services not available:', e.message);
}
...
class TradingAgent {
  constructor(db, options = {}) {
    ...
    // NEW: Tax-aware trading configuration
    this.taxAwareTrading = options.taxAwareTrading !== false; // Default to true
    this.taxJurisdiction = options.taxJurisdiction || 'AT'; // Default to Austria
    this._portfolioTaxService = null; // Lazy loaded
    this._taxRegime = null; // Cached tax regime

    // NEW: VIX and regime-based position scaling
    this.vixScalingEnabled = options.vixScalingEnabled !== false; // Default to true
    this.vixThreshold = options.vixThreshold || 25; // Start scaling down above this VIX level
    this.regimeScalingEnabled = options.regimeScalingEnabled !== false; // Default to true

    // NEW: Risk controls passed from agent config
    this.pauseInCrisis = options.pauseInCrisis !== false; // Default to true
  ...
  _getPortfolioTaxService() {
    if (!this._portfolioTaxService && PortfolioTaxService) {
      try {
        this._portfolioTaxService = new PortfolioTaxService(this.db, {
          jurisdiction: this.taxJurisdiction
        });
      } catch (error) {
        console.warn('Failed to initialize Portfolio Tax Service:', error.message);
        this._portfolioTaxService = null;
      }
    }
    return this._portfolioTaxService;
  }

  _getTaxRegime() {
    if (!this._taxRegime && getTaxRegime) {
      this._taxRegime = getTaxRegime(this.taxJurisdiction);
    }
    return this._taxRegime;
  }

  /**
   * Calculate tax impact for a potential sell trade
  ...
  async _calculateTaxImpact(symbol, shares, currentPrice, portfolioId = null) {
    ...
    const taxRegime = this._getTaxRegime();

    if (!taxService || !taxRegime) return null;
    ...
    try {
      ...
      return {
        estimatedGain: impact.estimatedGain || 0,
        estimatedTax: impact.estimatedTax || 0,
        effectiveTaxRate: impact.effectiveTaxRate || taxRegime.capitalGains.rate,
        netProceeds: impact.netProceeds || (shares * currentPrice),
        isLongTerm: impact.isLongTerm || false,
        holdingPeriodDays: impact.holdingPeriodDays || 0,
        taxRegime: taxRegime.name,
        hasWashSaleRisk: taxRegime.lossRules.washSaleRule && impact.estimatedGain < 0,
        recommendation: this._getTaxRecommendation(impact, taxRegime)
      };
    } catch (error) {
      console.warn('Tax impact calculation failed:', error.message);
      return null;
    }
  }

  /**
   * Generate tax-aware recommendation based on impact
   */
  _getTaxRecommendation(impact, taxRegime) {
    if (!impact) return null;

    const gain = impact.estimatedGain || 0;
    const tax = impact.estimatedTax || 0;

    // Check for long-term holding benefit (US)
    if (taxRegime.capitalGains.hasHoldingPeriodBenefit && !impact.isLongTerm) {
      const daysUntilLongTerm = taxRegime.capitalGains.holdingPeriodDays - (impact.holdingPeriodDays || 0);
      if (daysUntilLongTerm > 0 && daysUntilLongTerm < 90 && gain > 0) {
        return {
          action: 'consider_delay',
          reason: `Hold ${daysUntilLongTerm} more days for long-term rate (${(taxRegime.capitalGains.longTermRate * 100).toFixed(0)}% vs ${(taxRegime.capitalGains.shortTermRate * 100).toFixed(0)}%)`,
          potentialSavings: gain * (taxRegime.capitalGains.shortTermRate - taxRegime.capitalGains.longTermRate)
        };
      }
    }

    // Check for tax loss harvesting opportunity
    if (gain < -1000 && taxRegime.lossRules.canOffsetGains) {
      return {
        action: 'harvest_loss',
        reason: `Tax loss of ${Math.abs(gain).toFixed(0)} can offset other gains`,
        taxBenefit: Math.abs(gain) * taxRegime.capitalGains.rate
      };
    }

    // Check for wash sale risk (US/UK)
    if (taxRegime.lossRules.washSaleRule && gain < 0) {
      return {
        action: 'wash_sale_warning',
        reason: `Loss may be disallowed if repurchased within ${taxRegime.lossRules.washSaleWindowDays} days`,
  ...
  async getRecommendation(symbol, portfolioContext = null, regime = null) {
    ...
    // 6. Convert to action
    let action = this._scoreToAction(adjustedScore);

    // 6.5 P4 FIX: Regime blocking for weak buys (Expert Panel: Dalio, Taleb recommendations)
    // In BEAR/CRISIS regimes, weak buys should be blocked - only high conviction signals should proceed
    const bearishRegimes = ['BEAR', 'CRISIS', 'HIGH_VOL'];
    if (bearishRegimes.includes(marketRegime?.regime)) {
      const isBuyAction = action.action === 'buy' || action.action === 'strong_buy';
      const isWeakConviction = adjustedScore < 0.60; // Require score > 0.60 in bearish regimes

      if (isBuyAction && isWeakConviction) {
        // Convert weak buys to HOLD in bearish regimes
        action = { action: 'hold', confidence: 0.5 };
        // Log the blocking for transparency
        if (this.debug) {
          console.log(`[REGIME BLOCK] ${company.symbol}: BUY blocked in ${marketRegime.regime} (score: ${adjustedScore.toFixed(3)} < 0.60 required)`);
        }
      }
    }
    ...
    // 9.6 Calculate tax impact for sell recommendations
    let taxImpact = null;
    if (this.taxAwareTrading && action.action.includes('sell') && portfolioContext) {
      // Estimate shares to sell based on current position
      const currentPosition = portfolioContext.positions?.find(p => p.symbol === company.symbol);
      if (currentPosition && currentPosition.shares > 0) {
        const sharesToSell = action.action === 'strong_sell'
        ...
        // Add tax reasoning if significant
        if (taxImpact) {
          if (taxImpact.recommendation) {
            reasoning.push({
              factor: 'Tax Consideration',
              direction: taxImpact.recommendation.action === 'harvest_loss' ? 'bullish' : 'cautionary',
              weight: 0,
              details: taxImpact.recommendation.reason,
            });
          } else if (taxImpact.estimatedTax > 0) {
            reasoning.push({
              factor: 'Tax Impact',
              direction: 'informational',
              weight: 0,
              details: `Estimated tax: $${taxImpact.estimatedTax.toFixed(0)} (${(taxImpact.effectiveTaxRate * 100).toFixed(1)}% rate, ${taxImpact.taxRegime})`,
            });
          }
        }
      }
    ...
    const recommendation = {
      ...
      taxImpact, // NEW: Tax impact for sell recommendations
      timestamp: new Date().toISOString(),
      meta: {
        ...
        taxAwareTrading: this.taxAwareTrading,
   ...
   * P3 FIX: Rebalanced thresholds (Expert Panel: Buffett, Taleb recommendations)
   * Old: BUY >= 0.25 (too low, 90% of signals were BUY)
   * New: BUY >= 0.40, SELL <= -0.15 (more balanced, promotes HOLD as default)
   */
  _scoreToAction(score) {
    // STRONG_BUY: Only for high conviction signals (top tier)
    if (score >= 0.55) {
      return { action: ACTIONS.STRONG_BUY, confidence: Math.min(0.95, 0.55 + Math.abs(score) * 0.35) };
    }
    // BUY: Raised from 0.25 to 0.40 (significantly more selective)
    if (score >= 0.40) {
      return { action: ACTIONS.BUY, confidence: 0.5 + Math.abs(score) * 0.3 };
    }
    // STRONG_SELL: Strong negative conviction
    if (score <= -0.40) {
      return { action: ACTIONS.STRONG_SELL, confidence: Math.min(0.95, 0.55 + Math.abs(score) * 0.35) };
    }
    // SELL: Lowered from -0.25 to -0.15 (easier to trigger sells for balance)
    if (score <= -0.15) {
      return { action: ACTIONS.SELL, confidence: 0.5 + Math.abs(score) * 0.3 };
    }
    // HOLD: Default for uncertain signals (scores between -0.15 and 0.40)
    // This range was previously mostly BUY, now properly defaults to HOLD
  ...
  async _calculatePositionSize(score, portfolioContext, regime, signals = null) {
    ...
    // Apply regime-based position scaling (only if enabled)
    baseSize = this._applyRegimeScaling(baseSize, regime?.regime);

    // Apply VIX-based position scaling (only if enabled)
    baseSize = this._applyVixScaling(baseSize, regime?.vix);
  ...
  /**
   * Apply VIX-based position scaling
   * Reduces position sizes when VIX is elevated above threshold
   *
   * @param {number} positionSize - Base position size
   * @param {number} vix - Current VIX value
   * @returns {number} Adjusted position size
   */
  _applyVixScaling(positionSize, vix) {
    // If VIX scaling disabled or no VIX data, return unchanged
    if (!this.vixScalingEnabled || !vix) {
      return positionSize;
    }

    // If VIX below threshold, no adjustment needed
    if (vix <= this.vixThreshold) {
      return positionSize;
    }

    // Scale down as VIX increases above threshold
    // Formula: 2% reduction for each point above threshold, minimum 30% of original
    const vixExcess = vix - this.vixThreshold;
    const scalingFactor = Math.max(0.3, 1 - (vixExcess * 0.02));

    return positionSize * scalingFactor;
  }
   ...
   *
   * @param {string} positionSize - Base position size
   * @param {string} regime - Market regime (BULL, BEAR, SIDEWAYS, HIGH_VOL, CRISIS)
   * @returns {number} Adjusted position size
   */
  _applyRegimeScaling(positionSize, regime) {
    // If regime scaling disabled, return unchanged
    if (!this.regimeScalingEnabled) {
      return positionSize;
    }

    const regimeMultipliers = {
      'BULL': 1.0,      // Full position in bull markets
      'SIDEWAYS': 0.8,  // 80% in sideways markets
      'BEAR': 0.5,      // 50% in bear markets
      'HIGH_VOL': 0.4,  // 40% in high volatility
      'CRISIS': 0.3,    // 30% in crisis
    };

    const multiplier = regimeMultipliers[regime] || 0.7;
  ...
  async batchRecommendations(symbols, portfolioContext = null, regime = null) {
    ...
    // Filter and sort recommendations by score descending (strongest signals first)
    const recommendations = results.filter(r => !r.error);
    recommendations.sort((a, b) => {
      // Primary sort: by absolute score descending (strongest signals first)
      const scoreDiff = Math.abs(b.score || 0) - Math.abs(a.score || 0);
  ...
  /**
   * Set the tax jurisdiction for tax-aware trading
   * @param {string} jurisdiction - Tax jurisdiction code (AT, US, DE, UK, CH, NONE)
   */
  setTaxJurisdiction(jurisdiction) {
    this.taxJurisdiction = jurisdiction?.toUpperCase() || 'AT';
    this._taxRegime = null; // Clear cached regime
    this._portfolioTaxService = null; // Clear cached service to reinitialize with new jurisdiction
    console.log(`🤖 Tax jurisdiction set to: ${this.taxJurisdiction}`);
  }

  /**
   * Get tax loss harvesting opportunities for a portfolio
  ...
  /**
   * Get year-end tax summary for a portfolio
   * @param {number} portfolioId - Portfolio ID
  ...
  /**
   * Check if tax-aware trading is enabled and configured
   * @returns {object} Tax configuration status
   */
  getTaxConfiguration() {
    const regime = this._getTaxRegime();
    return {
      enabled: this.taxAwareTrading,
      jurisdiction: this.taxJurisdiction,
      regime: regime ? {
        name: regime.name,
        capitalGainsRate: regime.capitalGains.rate,
