...
class RiskManager {
  ...
  async validate(recommendation, portfolioId, regime = null) {
    ...
    // Approved if all passed OR (no critical failures AND >70% passed)
    const approved = allPassed || (!criticalFailed && passRate >= 0.7);
  ...
  async _checkDrawdown(portfolioId, portfolio) {
    ...
    return {
      ...
      critical: false, // Warning, not hard blocker
      value: Math.round(drawdown * 10000) / 100,
      limit: Math.round(this.limits.maxDrawdownPause * 100),
      peakValue: Math.round(peakValue * 100) / 100,
      currentValue: Math.round(currentValue * 100) / 100,
      message: inDrawdownPause
        ? `Portfolio in ${(drawdown * 100).toFixed(1)}% drawdown - consider pausing new positions`
        : null,
    };
  }
  ...
  async _checkStressTestLimits(portfolioId, recommendation) {
    try {
      ...
      return {
        ...
        passed,
        critical: false, // Warning, allows trade with reduced size
        worstScenario,
