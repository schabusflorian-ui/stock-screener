    ...
    this.stmts = {
      ...
      getCongressBuying: this.db.prepare(`
        SELECT
          ...
          SUM(CASE WHEN ct.transaction_type = 'purchase' THEN (ct.amount_low + COALESCE(ct.amount_high, ct.amount_low)) / 2 ELSE 0 END) as buy_amount,
          SUM(CASE WHEN ct.transaction_type = 'sale' THEN (ct.amount_low + COALESCE(ct.amount_high, ct.amount_low)) / 2 ELSE 0 END) as sell_amount,
    ...
    for (const symbol of symbols) {
      try {
        ...
        const company = this.db.prepare(`
          SELECT id, symbol, name, sector FROM companies WHERE symbol = ? COLLATE NOCASE
        `).get(symbol);
