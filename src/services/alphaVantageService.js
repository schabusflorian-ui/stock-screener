...
class AlphaVantageService {
  ...
  /**
   * Validate that income statement and balance sheet have consistent units
   * Detects common issues like balance sheet in billions while income in dollars
   *
   * @param {Object} income - Normalized income statement data
   * @param {Object} balance - Normalized balance sheet data
   * @returns {Object} { valid: boolean, reason?: string, warnings?: string[] }
   */
  validateUnits(income, balance) {
    const warnings = [];

    // Operating income should never exceed total assets (impossible ratio)
    if (income.operatingIncome && balance.totalAssets && balance.totalAssets > 0) {
      const ratio = income.operatingIncome / balance.totalAssets;
      if (ratio > 1) {
        console.warn(`⚠️ Unit mismatch detected: Operating Income (${income.operatingIncome}) > Total Assets (${balance.totalAssets})`);
        return {
          valid: false,
          reason: 'UNIT_MISMATCH',
          details: `Operating income / Total assets = ${ratio.toFixed(2)} (should be < 1)`
        };
      }
    }

    // Net margin should be between -500% and 100%
    if (income.netIncome && income.totalRevenue && income.totalRevenue > 0) {
      const netMargin = income.netIncome / income.totalRevenue;
      if (netMargin > 1) {
        warnings.push(`Suspicious net margin: ${(netMargin * 100).toFixed(1)}% (net income > revenue)`);
      }
      if (netMargin < -5) {
        warnings.push(`Extreme negative margin: ${(netMargin * 100).toFixed(1)}%`);
      }
    }

    // Total liabilities should not exceed total assets by more than 10x
    if (balance.totalLiabilities && balance.totalAssets && balance.totalAssets > 0) {
      const leverageRatio = balance.totalLiabilities / balance.totalAssets;
      if (leverageRatio > 10) {
        warnings.push(`Extreme leverage: Liabilities/Assets = ${leverageRatio.toFixed(1)}x`);
      }
    }

    // Gross profit should not exceed revenue
    if (income.grossProfit && income.totalRevenue && income.grossProfit > income.totalRevenue * 1.1) {
      warnings.push(`Gross profit (${income.grossProfit}) exceeds revenue (${income.totalRevenue})`);
    }
  ...
  async fetchAllData(symbol) {
    ...
    try {
      ...
      // Validate unit consistency between income statement and balance sheet
      // Check annual reports if available
      if (incomeStatement.annual?.length > 0 && balanceSheet.annual?.length > 0) {
        const latestIncome = incomeStatement.annual[0];
        const latestBalance = balanceSheet.annual[0];
        const validation = this.validateUnits(latestIncome, latestBalance);

        if (!validation.valid) {
          console.warn(`⚠️ ${symbol}: Unit validation failed - ${validation.reason}: ${validation.details}`);
          // Mark the data as potentially unreliable but don't throw
          // The metricCalculator will handle bounds checking
        }
      }
