...
class SECFilingFetcher {
  constructor(userAgent = 'Stock Analyzer contact@example.com') {
    ...
    // Circuit breaker for SEC API resilience
    this.circuitBreaker = registry.get('sec_edgar', {
      failureThreshold: 5,
      resetTimeout: 60000, // 1 minute (SEC rate limits are strict)
    });
  }
  ...
  /**
   * Generic fetch with rate limiting, circuit breaker, and error handling
   */
  async fetch(url, options = {}) {
    ...
    return this.circuitBreaker.execute(() => this._doFetch(url, options));
  }
