...
class EtfService {
  ...
  getEtfBySymbol(symbol) {
    return this.db.prepare(`
      SELECT * FROM etf_definitions WHERE symbol = ? COLLATE NOCASE
    `).get(symbol);
  }
  ...
  getModelPortfolio(modelIdOrName) {
    ...
    if (typeof modelIdOrName === 'string') {
      model = this.db.prepare(`
        SELECT * FROM model_portfolios WHERE name = ? COLLATE NOCASE
      `).get(modelIdOrName);
    } else {
      model = this.db.prepare(`
  ...
  createModelPortfolio(name, description, allocations, options = {}) {
    ...
    const getEtfId = this.db.prepare('SELECT id FROM etf_definitions WHERE symbol = ? COLLATE NOCASE');
  ...
  preparePortfolioFromModel(modelIdOrName, amount, options = {}) {
    ...
    // Filter allocations
    let allocations = model.allocations
