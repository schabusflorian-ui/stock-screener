...
async function routeQuery(query, context = {}) {
  const decision = shouldUseLLM(query, context);

  console.log(`[QueryRouter] Query: "${query.slice(0, 50)}..." -> ${decision.useLLM ? 'LLM' : 'Handler'} (${decision.reason})`);
  ...
  if (decision.useLLM) {
    ...
    // Process with LLM
    const result = await llmHandler.processQuery(query, conversationHistory, context);
