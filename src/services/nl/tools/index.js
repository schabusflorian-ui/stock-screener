...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Get historical price data for a SINGLE stock ticker. Returns ONE price chart. For 2+ stocks on ONE CHART, use compare_companies instead (it creates a combined overlay). Only use this tool for a single stock or when user explicitly asks for 'separate charts'. This tool only works with US stock tickers (AAPL, NVDA, MSFT, etc). Does NOT support indices (S&P 500, NASDAQ, DAX, etc) or international markets.",
    input_schema: {
      type: 'object',
      properties: {
        symbol: {
  ...
  },
  {
    ...
    description: "Compare two or more companies side by side on key metrics AND generate a combined price comparison chart. Use for: 'compare AAPL and MSFT', 'which is better NVDA or AMD', OR when user wants a SINGLE CHART showing multiple stock prices (e.g., 'one chart with Apple and Microsoft', 'plot them together'). Returns a multi-line overlay price chart alongside metric comparisons.",
    input_schema: {
      type: 'object',
      properties: {
        symbols: {
...
4. **Tool usage strategy:**
   - Usually one tool call is enough. Don't call multiple tools that return overlapping data.

## CRITICAL: MULTI-STOCK PRICE CHART RULES

**When user asks for prices of 2+ stocks:**

| User Request Pattern | Tool to Use | Result |
|---------------------|-------------|--------|
| "one chart with X and Y" | compare_companies | Single combined overlay chart |
| "compare X and Y prices" | compare_companies | Single combined overlay chart |
| "plot X and Y together" | compare_companies | Single combined overlay chart |
| "X and Y on the same chart" | compare_companies | Single combined overlay chart |
| "chart with X and Y" (no "separate") | compare_companies | Single combined overlay chart |
| "separate charts for X and Y" | get_price_history x2 | Two individual charts |

**DEFAULT BEHAVIOR**: If user mentions 2+ stocks and prices/charts without saying "separate", USE compare_companies. It generates ONE combined multi-line chart which is the better UX.

**DO NOT** call get_price_history multiple times unless user explicitly asks for "separate" or "individual" charts.
...
9. **CRITICAL: Avoid redundant tool calls in follow-ups:**
   - If you ALREADY called a tool in a previous turn and have that data, DO NOT call it again
   - For follow-up questions like "why did X behave this way?" - answer using the data you already retrieved
   - Only call a tool again if the user asks for NEW data (different time period, different metrics, different stocks)
   - Charts from previous turns are already visible to the user - returning the same charts again is redundant and confusing
   - Example: User asks "plot AAPL and NVDA", you call compare_companies. User then asks "why did AAPL drop?" → Answer from existing data, do NOT re-call compare_companies
...
User: "Give me one chart with Apple and Microsoft prices" / "Chart with AAPL and GOOGL"
→ Use compare_companies (returns ONE combined multi-line overlay chart)
→ Response: "Here's a combined chart comparing AAPL and MSFT price performance."

User: "Show me Apple and Microsoft prices" / "Price chart for AAPL and MSFT"
→ Use compare_companies (DEFAULT for 2+ stocks = combined chart, better UX)
→ Response: "Here's a combined price comparison chart for AAPL and MSFT."

User: "Show me the price for Apple" (single stock)
→ Use get_price_history once for AAPL
→ Response: "Here's Apple's price chart for the past 90 days."

User: "Show me SEPARATE charts for Apple and Microsoft" (user explicitly says separate)
→ Call get_price_history TWICE for individual charts
→ Response: "Here are the individual price charts for AAPL and MSFT."
