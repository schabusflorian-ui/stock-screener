  ...
  async getSectorOverview(periodType = 'annual') {
    ...
    const sql = usePm
      ...
      SELECT
        ...
        ROUND(AVG(CASE WHEN roa BETWEEN -50 AND 100 THEN roa END)::numeric, 2) as avg_roa,
        ROUND(AVG(CASE WHEN gross_margin BETWEEN -50 AND 100 THEN gross_margin END)::numeric, 2) as avg_gross_margin,
        ROUND(AVG(CASE WHEN operating_margin BETWEEN -50 AND 100 THEN operating_margin END)::numeric, 2) as avg_operating_margin,
        ROUND(AVG(CASE WHEN net_margin BETWEEN -50 AND 100 THEN net_margin END)::numeric, 2) as avg_net_margin,
        ...
        ROUND(AVG(CASE WHEN ev_ebitda BETWEEN 0 AND 100 THEN ev_ebitda END)::numeric, 2) as avg_ev_ebitda,
        ROUND(AVG(CASE WHEN debt_to_equity BETWEEN 0 AND 10 THEN debt_to_equity END)::numeric, 2) as avg_debt_to_equity,
        ROUND(AVG(CASE WHEN current_ratio BETWEEN 0 AND 20 THEN current_ratio END)::numeric, 2) as avg_current_ratio,
  ...
  async getTopPerformersBySector(metric = 'roic', limit = 5, periodType = 'annual') {
    ...
    const sql = usePm
      ...
      SELECT
        ...
        name,
        industry,
        ROUND(roic::numeric, 2) as roic,
        ROUND(roe::numeric, 2) as roe,
        ROUND(net_margin::numeric, 2) as net_margin,
    ...
    const sql = `
      WITH sector_periods AS (
        SELECT
          ...
          ROUND(AVG(CASE WHEN m.roic BETWEEN -50 AND 200 THEN m.roic END)::numeric, 2) as avg_roic,
          ROUND(AVG(CASE WHEN m.roe BETWEEN -50 AND 200 THEN m.roe END)::numeric, 2) as avg_roe,
          ROUND(AVG(CASE WHEN m.net_margin BETWEEN -50 AND 100 THEN m.net_margin END)::numeric, 2) as avg_net_margin,
          ROUND(AVG(CASE WHEN m.revenue_growth_yoy BETWEEN -100 AND 500 THEN m.revenue_growth_yoy END)::numeric, 2) as avg_revenue_growth,
          ROUND(AVG(CASE WHEN m.fcf_yield BETWEEN -100 AND 100 THEN m.fcf_yield END)::numeric, 2) as avg_fcf_yield
        FROM calculated_metrics m
        JOIN companies c ON m.company_id = c.id
        WHERE m.period_type = $1
          AND c.sector IS NOT NULL
  ...
  async getIndustryMarginComparison(periodType = 'annual') {
    ...
    const sql = `
      ...
      SELECT
        ...
        ROUND(MAX(CASE WHEN gross_margin BETWEEN -100 AND 100 THEN gross_margin END)::numeric, 2) as max_gross_margin,

        ROUND(AVG(CASE WHEN operating_margin BETWEEN -100 AND 100 THEN operating_margin END)::numeric, 2) as avg_operating_margin,
        ROUND(MIN(CASE WHEN operating_margin BETWEEN -100 AND 100 THEN operating_margin END)::numeric, 2) as min_operating_margin,
        ROUND(MAX(CASE WHEN operating_margin BETWEEN -100 AND 100 THEN operating_margin END)::numeric, 2) as max_operating_margin,

        ROUND(AVG(CASE WHEN net_margin BETWEEN -100 AND 100 THEN net_margin END)::numeric, 2) as avg_net_margin,
  ...
  async getSectorDetail(sector, periodType = 'annual') {
    ...
    const sql = usePm
      ...
      SELECT
        ...
        ROUND(change_1w::numeric, 2) as change_1w,
        ROUND(change_1m::numeric, 2) as change_1m,
        ROUND(change_ytd::numeric, 2) as change_ytd,
        ROUND(change_1y::numeric, 2) as change_1y,
