...
class TCAResultsManager {
  ...
  _prepareStatements() {
    ...
    this.stmtGetByDateRange = this.db.prepare(`
      SELECT * FROM tca_benchmark_results
    ...
    this.stmtGetByStatus = this.db.prepare(`
      SELECT * FROM tca_benchmark_results
      WHERE overall_pass = ?
      ORDER BY created_at DESC
      LIMIT ?
    ...
    this.stmtGetSummaryStats = this.db.prepare(`
      SELECT
        COUNT(*) as total_runs,
    ...
    this.stmtGetTrend = this.db.prepare(`
      SELECT
   ...
   */
  getLatest() {
    const row = this.stmtGetLatest.get();
  ...
  getByDateRange(startDate, endDate) {
    const rows = this.stmtGetByDateRange.all(startDate, endDate);
   ...
   */
  getRecent(limit = 30) {
    const rows = this.stmtGetRecent.all(limit);
    return rows.map(row => this._parseRow(row));
  }
   ...
   * @returns {Array} Array of results
   */
  getByStatus(passed, limit = 30) {
    const rows = this.stmtGetByStatus.all(passed ? 1 : 0, limit);
    return rows.map(row => this._parseRow(row));
  }

  /**
   * Get summary statistics over a time period
   * @param {string} period - SQLite date modifier (e.g., '-30 days', '-90 days')
   * @returns {Object} Summary statistics
   */
  getSummaryStats(period = '-30 days') {
    const row = this.stmtGetSummaryStats.get(period);
   ...
   */
  getTrend(period = '-30 days') {
    const rows = this.stmtGetTrend.all(period);
    return rows.map(row => ({
      date: row.date,
   ...
   */
  getComparison(currentPeriod = '-30 days', previousPeriod = '-60 days') {
    const current = this.getSummaryStats(currentPeriod);
    const previous = this.getSummaryStats(previousPeriod);
