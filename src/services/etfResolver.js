...
class ETFResolver {
  constructor() {
    ...
    // In-memory cache for frequently accessed ETFs
    this.cache = new Map();
    this.CACHE_TTL = 5 * 60 * 1000; // 5 minutes

    console.log('ETFResolver initialized');
  }
  ...
  async resolve(symbol) {
    ...
    if (dbResult) {
      ...
      this.cache.set(upperSymbol, { data: etf, timestamp: Date.now() });
      return etf;
    }
    ...
    if (staticPortfolio) {
      ...
      const enrichedAllocations = staticPortfolio.allocations.map(alloc => {
        const etf = this.db.prepare(`
          SELECT name, expense_ratio, category FROM etf_definitions WHERE symbol = ?
        `).get(alloc.symbol);
