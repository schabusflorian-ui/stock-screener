...
class PositionSizing {
  ...
  _volatilityBased(params) {
    ...
    if (metrics?.volatility_30d) {
      stockVolatility = metrics.volatility_30d;
    } else {
    ...
    const weightPct = targetVolatility / stockVolatility;
    const positionValue = portfolioValue * Math.min(weightPct, 1); // Cap at 100%
    const positionPct = (positionValue / portfolioValue) * 100;
