...
class ScreeningService {
  ...
  /**
   * Clear the filter options cache (call after data updates)
   */
  ...
  async getFilterOptions() {
    ...
    // Cache the result
    this._filterOptionsCache = result;
    this._filterOptionsCacheTime = Date.now();
  ...
  async screen(criteria = {}) {
    const {
      ...
      // PRISM Score criteria (AI-generated investment quality, 1-5 scale)
      minPRISMScore, maxPRISMScore,
    ...
    addPrismCriteria('overall_score', minPRISMScore, maxPRISMScore);
    ...
    // Handle sorting by company fields vs metric fields vs price_metrics fields vs prism_scores
    const priceMetricsCols = [
      'market_cap', 'beta', 'enterprise_value', 'last_price',
      'change_1d', 'change_1w', 'change_1m', 'change_ytd',
      'alpha_1d', 'alpha_1w', 'alpha_1m', 'alpha_3m', 'alpha_6m', 'alpha_ytd', 'alpha_1y'
    ];
    const prismCols = ['prism_score'];
    let sortColumn;
    ...
    } else if (prismCols.includes(safeSortBy)) {
      sortColumn = 'ps.overall_score';
    } else {
