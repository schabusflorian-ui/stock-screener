...
class NotificationService {
  ...
  _getDefaultPreferences() {
    return {
      ...
      // SMART ALERTS: Default to watchlist-only to reduce noise
      watchlist_only: 1,
      portfolio_only: 0,
      custom_rules: '[]'
    };
  ...
  shouldSuggestWatchlistOnly(userId = this.options.defaultUserId) {
    ...
    if (prefs.watchlistOnly) {
      return { suggest: false, reason: 'already_enabled' };
    }
    ...
    if (alertCount.count >= 50) {
      return {
        suggest: true,
        reason: 'high_volume',
        currentCount: alertCount.count,
        message: `You received ${alertCount.count} alerts this week. Enable watchlist-only mode to focus on stocks you care about.`
      };
    }

    return { suggest: false, reason: 'low_volume' };
  }
  ...
  getAlertVolumeStats(userId = this.options.defaultUserId) {
    ...
    const stats = this.db.prepare(`
      SELECT
        ...
        COUNT(CASE WHEN created_at > ? THEN 1 END) as alerts_30d,
        COUNT(CASE WHEN created_at > ? AND status = 'unread' THEN 1 END) as unread_7d,
    ...
    return {
      ...
      isHighVolume: stats.alerts_7d >= 50,
      isLowEngagement: parseFloat(dismissRate) > 50
    };
  }
}
