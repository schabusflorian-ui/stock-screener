...
class HistoricalAgentBacktester {
   ...
   */
  async _getUniverse() {
    const { universe, minMarketCap } = this.config;

    if (Array.isArray(universe)) {
      // Custom symbol list
      return universe.map(symbol => {
        const company = this.stmtGetCompany.get(symbol);
        return company ? { id: company.id, symbol, sector: company.sector, marketCap: company.market_cap } : null;
      }).filter(Boolean);
    }
