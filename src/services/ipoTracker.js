...
class IPOTracker {
  ...
  ensureCompanyExists(ipo) {
    ...
    // For EU/UK IPOs without ticker, try alternative approaches
    if (!ticker && ipo.isin) {
    ...
    if (!ticker && ipo.company_name) {
      // Generate ticker from company name (first 5 uppercase letters)
      ticker = ipo.company_name.replace(/[^A-Za-z]/g, '').substring(0, 5).toUpperCase();
    }

    if (!ticker) {
      console.log(`  Cannot create company for IPO ${ipo.id}: no ticker could be determined`);
      return null;
    }
