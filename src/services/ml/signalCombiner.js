...
class GradientBoostingRegressor {
  ...
  fit(X, y) {
    ...
    const predictions = new Array(nSamples).fill(this.initialPrediction);
...
class MLSignalCombiner {
  ...
  /**
   * Get feature importance for a given horizon (public API)
   * @param {number} horizon Target horizon in days (default: 21)
   * @returns {Object} Feature importance map
   */
  getFeatureImportance(horizon = 21) {
    if (!this.models[horizon]) {
      // Return default weights if no model
      return {
        technical: 0.12,
        sentiment: 0.08,
        insider: 0.15,
    ...
    return this._getFeatureImportanceMap(this.models[horizon]);
  }
