...
class TrendAnalysis {
  ...
  findBestTrends(minScore = 3) {
    ...
    const allMetrics = this.db.prepare(`
      ...
      FROM companies c
      INNER JOIN calculated_metrics cm ON c.id = cm.company_id
      WHERE c.is_active = 1
    ...
    for (const row of allMetrics) {
      if (!companyMetricsMap.has(row.symbol)) {
        companyMetricsMap.set(row.symbol, {
          company: { symbol: row.symbol, name: row.name, sector: row.sector },
          metrics: []
        });
      }
      companyMetricsMap.get(row.symbol).metrics.push(row);
    }

    const improving = [];

    // Process each company's metrics in memory (no additional queries)
    for (const [symbol, data] of companyMetricsMap) {
      const history = data.metrics;

      // Need at least 2 data points for trend analysis
      if (history.length < 2) continue;

      // Build trends object (same structure as getCompanyTrends)
      const trends = {
