...
const { getDb, tableExists, columnExists } = require('./_migrationHelper');
...
if (tableExists(db, 'portfolios') && !columnExists(db, 'portfolios', 'user_id')) {
...
} else if (tableExists(db, 'portfolios')) {
  console.log('portfolios.user_id already exists, skipping');
}

// Add user_id to notes if exists
if (tableExists(db, 'notes') && !columnExists(db, 'notes', 'user_id')) {
...
if (tableExists(db, 'theses') && !columnExists(db, 'theses', 'user_id')) {
