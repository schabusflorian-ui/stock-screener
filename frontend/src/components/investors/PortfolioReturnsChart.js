...
export function PortfolioReturnsChart({ investorId, investorName }) {
    ...
    async function fetchReturns() {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          setError(res.data.error || 'No returns data available');
        }
      } catch (err) {
        console.error('Error fetching returns:', err);
        // Extract error message from API response if available
        const apiError = err.response?.data?.error;
        if (apiError?.includes('Not enough data')) {
          setError('Need at least 2 quarterly filings to calculate returns. Only 1 filing period available.');
        } else {
          setError(apiError || 'Failed to load returns data');
        }
      } finally {
        setLoading(false);
      }
  ...
  if (error || !data || !data.returns || data.returns.length === 0) {
    const isInsufficientData = error?.includes('filing') || error?.includes('Not enough');
    return (
      <div className="returns-chart-container">
        <div className="returns-chart-empty">
          ...
          <span className="returns-chart-hint">
            {isInsufficientData
              ? 'Returns require comparing holdings across multiple 13F filing periods. Once more filings are available, performance will be calculated automatically.'
  ...
  return (
    <div className="returns-chart-container" {...askAIProps}>
      {/* Header */}
      ...
      <div className="returns-chart-info">
        <span>
          Returns calculated from weighted price changes of disclosed 13F holdings (public stocks only).
          Does not include private businesses, cash positions, derivatives, or intra-quarter trading.
          For conglomerates like Berkshire Hathaway, 13F holdings may represent only 25-35% of total enterprise value.
        </span>
      </div>
    </div>
  );
