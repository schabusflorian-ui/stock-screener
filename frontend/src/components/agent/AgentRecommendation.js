...
import { TrendingUp, TrendingDown, Minus, AlertCircle, Clock } from '../icons';
 ...
 * Note: Using "bullish/bearish" terminology for regulatory compliance (BaFin/FMA/MiFID II)
 */
const ACTION_CONFIG = {
  strong_buy: { label: 'Strong Bullish', variant: 'green', icon: TrendingUp },
  strong_bullish: { label: 'Strong Bullish', variant: 'green', icon: TrendingUp },
  buy: { label: 'Bullish', variant: 'green', icon: TrendingUp },
  bullish: { label: 'Bullish', variant: 'green', icon: TrendingUp },
  hold: { label: 'Neutral', variant: 'gray', icon: Minus },
  neutral: { label: 'Neutral', variant: 'gray', icon: Minus },
  sell: { label: 'Bearish', variant: 'red', icon: TrendingDown },
  bearish: { label: 'Bearish', variant: 'red', icon: TrendingDown },
  strong_sell: { label: 'Strong Bearish', variant: 'red', icon: TrendingDown },
  strong_bearish: { label: 'Strong Bearish', variant: 'red', icon: TrendingDown },
};
...
function AgentRecommendation({ symbol, portfolioId, className = '' }) {
  ...
  return (
    <Card variant="glass" className={`agent-recommendation ${className}`}>
      {/* Compliance Banner */}
      <ComplianceDisclaimer variant="compact" type="ai" />

      {/* Header */}
      <div className="agent-recommendation__header">
        <div className="agent-recommendation__title">
          <h3>{symbol}</h3>
          <span className="agent-recommendation__subtitle">AI Analysis</span>
        </div>
      ...
      {/* Position Size - Model Calculation */}
      {recommendation.position_size && (
        <div className="agent-recommendation__position">
          Model-Calculated Allocation: <strong>{(recommendation.position_size * 100).toFixed(1)}%</strong> of portfolio
          {recommendation.suggested_value && (
            <span className="agent-recommendation__position-value">
              (~${recommendation.suggested_value.toLocaleString()} virtual)
            </span>
          )}
        </div>
      )}
      ...
      <ComplianceDisclaimer variant="inline" type="general" />
    </Card>
  );
}
