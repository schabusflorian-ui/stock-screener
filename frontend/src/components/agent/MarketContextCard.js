...
const REGIME_CONFIG = {
  BULL: {
    ...
    color: 'success',
    label: 'Bull Market',
    description: 'Favorable conditions for long positions'
  },
  BEAR: {
    icon: TrendingDown,
    color: 'danger',
  ...
  HIGH_VOL: {
    ...
    description: 'Increased uncertainty, smaller positions advised'
  },
  LOW_VOL: {
    icon: Minus,
    color: 'primary',
    label: 'Low Volatility',
    description: 'Stable conditions, normal positioning'
  },
  NEUTRAL: {
    icon: Activity,
    color: 'default',
    label: 'Neutral',
    description: 'Mixed signals, await clearer trend'
  },
  CRISIS: {
    icon: AlertTriangle,
    color: 'danger',
    label: 'Crisis Mode',
    description: 'Risk-off, cash preservation priority'
  }
};
...
function MetricRow({ icon: Icon, label, value, subvalue, color }) {
  return (
    <div className="market-context__metric">
      <div className={`market-context__metric-icon market-context__metric-icon--${color || 'default'}`}>
        <Icon size={14} />
      </div>
