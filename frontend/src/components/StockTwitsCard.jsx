...
import { MessageSquare, TrendingUp, TrendingDown, Minus, RefreshCw, ExternalLink } from './icons';
...
export function StockTwitsCard({ symbol, data: propData, onRefresh, loading: propLoading }) {
  ...
  // Ask AI context menu for StockTwits sentiment
  const askAIProps = useAskAI(() => ({
    type: 'metric',
    metric: 'stocktwits_sentiment',
    symbol,
    label: 'StockTwits Sentiment',
    signal: data?.sentiment?.signal,
    confidence: data?.sentiment?.confidence,
    bullishCount: data?.stats?.bullishCount,
    bearishCount: data?.stats?.bearishCount
  }));
  ...
  if ((loading || propLoading) && !data) {
    return (
      <div className="stocktwits-card stocktwits-card--loading" {...askAIProps}>
  ...
  if (!data || error) {
    return (
      <div className="stocktwits-card stocktwits-card--empty" {...askAIProps}>
        <div className="stocktwits-header">
          <div className="header-title">
            <MessageSquare size={16} />
            <h3>StockTwits</h3>
  ...
  return (
    <div className="stocktwits-card" {...askAIProps}>
      {/* Header */}
      <div className="stocktwits-header">
        <div className="header-title">
          <MessageSquare size={16} />
...
export default memo(StockTwitsCard);
