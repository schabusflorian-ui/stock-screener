...
function MLCombinerPanel({ agentId, onMLStatusChange }) {
  ...
  const handleTrain = async () => {
    try {
      ...
      if (!res?.data) {
        throw new Error('No response received from ML training API');
      }

      if (!res.data.success) {
        throw new Error(res.data.error || 'Training failed - check if training data exists in database');
      }

      if (!res.data.data) {
        throw new Error('Training completed but returned no results');
      }
      ...
      setError(err.response?.data?.error || err.message || 'Unknown training error occurred');
    } finally {
      setTraining(false);
    }
  };
