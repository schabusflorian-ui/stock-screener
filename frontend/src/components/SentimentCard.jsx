...
export function SentimentCard({ data, onRefresh, loading, symbol }) {
  ...
  // Ask AI context menu for Reddit sentiment
  const askAIProps = useAskAI(() => ({
    ...
    sentiment: data?.analysis?.weightedSentiment,
  ...
  }, [showChart, symbol, historyData.length, loadHistory]);

  if (!data || !data.analysis) {
    return (
      <div className="sentiment-card sentiment-card--empty" {...askAIProps}>
  ...
  return (
    <div className="sentiment-card" {...askAIProps}>
      {/* Header with Signal */}
      <div className="sentiment-header">
        <h3>Reddit Sentiment</h3>
