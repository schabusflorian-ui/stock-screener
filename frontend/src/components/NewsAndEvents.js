...
import { BarChart2, TrendingUp, Newspaper, User, FileText, Landmark, Rocket } from './icons';
import './NewsAndEvents.css';

// Filing type to Prism icon mapping
const FILING_ICONS = {
  '10-K': BarChart2,
  '10-Q': TrendingUp,
  '8-K': Newspaper,
...
// Get filing icon component based on form type
const getFilingIcon = (formType) => {
  return FILING_ICONS[formType] || FILING_ICONS.default;
};
...
function NewsAndEvents({ symbol }) {
  ...
  // Ask AI context menu for news and events
  const askAIProps = useAskAI(() => ({
    type: 'metric',
    metric: 'news_events',
    symbol,
    label: 'News & Events',
    newsCount: newsData?.news?.data?.length,
    filingsCount: newsData?.secFilings?.data?.length,
    activeTab
  }));
  ...
  if (loading) {
    return (
      <div className="news-events-container" {...askAIProps}>
...
function FilingItem({ item }) {
  ...
  const FilingIcon = getFilingIcon(item.formType);
  ...
  return (
    ...
    >
      <div className="filing-icon"><FilingIcon size={18} /></div>
