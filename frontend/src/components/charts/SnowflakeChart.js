...
const DIMENSIONS = [
  { key: 'value', label: 'Value', color: '#6366f1', description: 'P/E, P/B, PEG ratios vs peers' },
  { key: 'growth', label: 'Growth', color: '#10b981', description: 'Revenue & earnings growth' },
  { key: 'past', label: 'Past', color: '#f59e0b', description: 'Historical performance' },
  { key: 'health', label: 'Health', color: '#3b82f6', description: 'Financial stability' },
  { key: 'dividend', label: 'Dividend', color: '#ec4899', description: 'Yield and growth' },
];
...
function getOverallInterpretation(total) {
  ...
  if (total >= 20) return { label: 'Good', color: '#22c55e' };
  if (total >= 15) return { label: 'Average', color: 'var(--warning)' };
  if (total >= 10) return { label: 'Below Average', color: '#f97316' };
...
function SnowflakeChart({ metrics, sectorAverage, size = 'medium', showLegend = true }) {
  ...
  return (
    <div className={`snowflake-chart size-${size}`}>
