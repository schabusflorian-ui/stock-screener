...
function AnalystActivityPanel() {
  ...
  // Ask AI context for analyst activity panel
  const askAIProps = useAskAI(() => ({
    type: 'metric',
    metric: 'analyst_activity',
    label: 'Analyst Activity',
    recentChangesCount: data?.recentChanges?.length || 0,
    upgrades: data?.recentChanges?.filter(c => c.action === 'upgrade').length || 0,
    downgrades: data?.recentChanges?.filter(c => c.action === 'downgrade').length || 0,
    strongBuysCount: data?.strongBuys?.length || 0,
    topStrongBuy: data?.strongBuys?.[0]?.symbol
  }));
  ...
  return (
    <div className="analyst-activity-panel" {...askAIProps}>
