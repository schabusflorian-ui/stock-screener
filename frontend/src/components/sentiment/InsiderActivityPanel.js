...
function InsiderActivityPanel({ days = 30 }) {
  ...
  // Ask AI context for insider activity panel
  const askAIProps = useAskAI(() => ({
    type: 'metric',
    metric: 'insider_activity',
    label: 'Insider Trading Activity',
    period: `${days} days`,
    totalBuys: data?.overview?.totalBuys,
    totalSells: data?.overview?.totalSells,
    buyToSellRatio: data?.overview?.buyToSellRatio,
    companiesWithActivity: data?.overview?.companiesWithActivity,
    topNetBuying: data?.netBuying?.[0]?.symbol,
    topNetSelling: data?.netSelling?.[0]?.symbol
  }));
  ...
  return (
    <div className="insider-activity-panel" {...askAIProps}>
      <div className="panel-header">
        <h3><Users size={18} /> Insider Activity</h3>
