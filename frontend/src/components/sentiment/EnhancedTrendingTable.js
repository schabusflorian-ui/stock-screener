...
const ExpandedRowPanel = ({ ticker, onClose }) => {
  ...
  useEffect(() => {
    const loadDetails = async () => {
      setLoading(true);
      try {
        // Load posts and analyst data in parallel for better performance
        const [postsRes, analystRes] = await Promise.all([
          sentimentAPI.getPostsForTicker(ticker.symbol, '7d', 5).catch(() => ({ data: { posts: [] } })),
          pricesAPI.getMetrics(ticker.symbol).catch(() => null)
        ]);
    ...
    return (
      <div className="enhanced-trending-table error">
        <div className="error-state">
          <RefreshCw size={20} />
          <span>Unable to load trending data</span>
    ...
    return (
      <div className="enhanced-trending-table">
        <div className="no-data">
          No trending tickers found for this period and region
        </div>
      </div>
    );
  }
            ...
            {sortedData.map(ticker => (
              <React.Fragment key={ticker.symbol}>
                <tr
                  ...
                  onContextMenu={(e) => handleRowContextMenu(e, ticker)}
                  data-ask-ai="true"
                >
