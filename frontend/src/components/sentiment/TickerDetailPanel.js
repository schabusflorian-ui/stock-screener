...
function TickerDetailPanel({ ticker, onClose }) {
  ...
  // Ask AI context for sentiment panel
  const askAIProps = useAskAI(() => ({
    ...
    value: data.combined?.combined?.sentiment
      ? `${(data.combined.combined.sentiment * 100).toFixed(0)}`
      : ticker?.avgSentiment
        ? `${(ticker.avgSentiment * 100).toFixed(0)}`
        : 'N/A',
    redditSentiment: data.combined?.sources?.reddit?.sentiment,
    stocktwitsSentiment: data.combined?.sources?.stocktwits?.sentiment,
    newsSentiment: data.combined?.sources?.news?.sentiment,
    analystConsensus: data.analyst?.buyPercent,
    upsidePotential: data.analyst?.upsidePotential
  }));
  ...
  return (
    <div className="ticker-detail-panel">
      ...
      {loading ? (
        ...
        <div className="panel-content" {...askAIProps}>
