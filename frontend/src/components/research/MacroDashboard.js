...
// Category colors - Prism Design System hex values
// Note: Recharts SVG components require hex values (CSS variables not supported in SVG)
// These values match the design system tokens:
// - rates: --info (#2563EB)
// - credit: --warning-dark (#D97706)
// - volatility: --negative (#DC2626)
// - employment: --positive (#059669)
// - growth: --color-ai-violet (#7C3AED)
// - inflation: --negative (#DC2626)
// - housing: --color-ai-cyan (#0891B2)
// - sentiment: --color-navy-400 (#94A3B8)
// - commodities: --warning-dark (#D97706)
// - money: --color-ai-violet (#7C3AED)
// - market: --color-ai-cyan (#0891B2)
    ...
    <div className="macro-dashboard">
      ...
      <div className="macro-section">
        ...
        <div className="key-metrics-grid">
          ...
          <AskAIProvider value={{
            ...
            data: {
              fedFunds: keyMetrics?.rates?.fedFunds,
              treasury2y: keyMetrics?.rates?.treasury2y,
              treasury10y: keyMetrics?.rates?.treasury10y,
              spread2s10s: keyMetrics?.rates?.spread2s10s,
              curveInverted: keyMetrics?.rates?.curveInverted
            }
          }}>
            <div className="metric-card rates" data-ask-ai="true">
          ...
          <AskAIProvider value={{
            ...
            value: keyMetrics?.volatility?.vix,
            data: {
              vix: keyMetrics?.volatility?.vix,
          ...
          <AskAIProvider value={{
            type: 'metric',
            metric: 'credit_markets',
            label: 'Credit Markets',
            value: keyMetrics?.credit?.hySpread,
            data: {
          ...
          <AskAIProvider value={{
            ...
            metric: 'employment',
            label: 'Employment',
            value: keyMetrics?.economy?.unemployment,
            data: {
              unemployment: keyMetrics?.economy?.unemployment,
              nfp: keyMetrics?.economy?.nfp,
              laborForceParticipation: keyMetrics?.economy?.laborForceParticipation
            }
          }}>
            <div className="metric-card economy" data-ask-ai="true">
              <div className="metric-header">
                <Briefcase size={18} />
      ...
      <div className="charts-row">
        ...
        <AskAIProvider value={{
          ...
          metric: 'yield_curve',
          label: 'Yield Curve',
          data: yieldCurve?.maturities?.reduce((acc, m) => { acc[m.term] = m.yield; return acc; }, {}),
          isInverted: yieldCurve?.is_inverted_2s10s === 1,
          interpretation: yieldCurve?.is_inverted_2s10s === 1
            ? 'Yield curve is INVERTED (2Y > 10Y) - historically a recession indicator'
            : 'Yield curve is normal (upward sloping)'
        }}>
        ...
        <AskAIProvider value={{
          type: 'chart',
          metric: 'spread_2s10s',
          label: '2s10s Spread History',
          currentValue: yieldCurveHistory?.[yieldCurveHistory.length - 1]?.spread_2s10s,
          trend: yieldCurveHistory?.length > 5
            ? (yieldCurveHistory[yieldCurveHistory.length - 1]?.spread_2s10s > yieldCurveHistory[yieldCurveHistory.length - 5]?.spread_2s10s ? 'steepening' : 'flattening')
            : undefined,
          periodDays: yieldCurveHistory?.length,
          isInverted: yieldCurveHistory?.[yieldCurveHistory.length - 1]?.spread_2s10s < 0
        }}>
          <div className="macro-section chart-section" data-ask-ai="true">
      ...
      <div className="charts-row">
        ...
        <AskAIProvider value={{
          ...
          metric: 'vix_history',
          label: 'VIX History',
          currentValue: vixHistory?.[vixHistory.length - 1]?.value,
          range: vixHistory?.length > 0 ? {
            min: Math.min(...vixHistory.map(v => v.value)),
            max: Math.max(...vixHistory.map(v => v.value)),
        ...
        <AskAIProvider value={{
          type: 'chart',
          metric: 'credit_spread_history',
          label: 'High-Yield Spread History',
          currentValue: creditHistory?.[creditHistory.length - 1]?.value,
          range: creditHistory?.length > 0 ? {
            min: Math.min(...creditHistory.map(v => v.value)),
            max: Math.max(...creditHistory.map(v => v.value)),
      ...
      {indicatorChartData.length > 0 && (
        <AskAIProvider value={{
          type: 'chart',
          metric: 'monthly_changes',
          label: 'Biggest Monthly Changes',
          data: indicatorChartData.map(d => ({ name: d.name, change: d.value })),
          topMover: indicatorChartData[0]?.name,
          topMoverChange: indicatorChartData[0]?.value
        }}>
          <div className="macro-section" data-ask-ai="true">
