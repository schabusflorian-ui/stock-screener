...
import {
  ...
  Minus, Info, CheckCircle, XCircle, Star, MoreHorizontal, HelpCircle
} from '../icons';
...
// Tooltips for Alpha analytics concepts
const ALPHA_TOOLTIPS = {
  jensensAlpha: "Excess return above what CAPM predicts based on your portfolio's risk level. Positive = outperformance.",
  multifactor: "Alpha after accounting for exposure to market, size, value, and momentum factors.",
  skillConfidence: "Statistical confidence that your returns are due to skill rather than luck.",
  informationRatio: "Risk-adjusted alpha. Higher is better. Above 0.5 is good, above 1.0 is excellent.",
  consistency: "Percentage of periods where you generated positive alpha."
};

// Period options with hints
const PERIOD_OPTIONS = [
  { value: '6m', label: '6 Months', hint: 'Recent performance' },
  { value: '1y', label: '1 Year', hint: 'Standard analysis period' },
  { value: '2y', label: '2 Years', hint: 'Medium-term view' },
  { value: '3y', label: '3 Years', hint: 'More statistical significance' },
  { value: '5y', label: '5 Years', hint: 'Full market cycle' }
];
...
function AlphaAnalyticsPanel({ portfolioId }) {
  ...
  // Ask AI context for alpha analytics
  const askAIProps = useAskAI(() => ({
    type: 'metric',
    metric: 'alpha_analytics',
    label: 'Alpha Analytics',
    period,
    benchmark,
    jensensAlpha: alphaData?.jensensAlpha,
    multifactorAlpha: alphaData?.multifactorAlpha,
    informationRatio: alphaData?.informationRatio,
    skillConfidence: alphaData?.skillConfidence,
    alphaRating: alphaData?.rating
  }));
  ...
  return (
    <div className="simulation-panel alpha-analytics-panel" {...askAIProps}>
      {/* Panel Header */}
      <div className="panel-header">
        <div className="header-content">
          <Award size={20} className="header-icon" />
          <div>
            <h3>Alpha Analytics</h3>
            <p className="panel-description">
              Measure your portfolio's skill-based outperformance vs market benchmarks
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="alpha-controls">
        <div className="control-group">
          <label className="form-label-enhanced">
            Analysis Period
            <HelpCircle size={12} className="help-icon" title="Longer periods provide more statistical significance" />
          </label>
          <select
            value={period}
          ...
          <span className="form-hint">{PERIOD_OPTIONS.find(p => p.value === period)?.hint}</span>
        </div>
