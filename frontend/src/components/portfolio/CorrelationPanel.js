...
import {
  ...
  Layers, PieChart, AlertCircle, ChevronDown, Info, HelpCircle
} from '../icons';
...
const PERIODS = [
  { value: '3m', label: '3 Months', hint: 'Recent market conditions' },
  { value: '6m', label: '6 Months', hint: 'Short-term trends' },
  { value: '1y', label: '1 Year', hint: 'Recommended baseline' },
  { value: '3y', label: '3 Years', hint: 'Long-term patterns' }
];

// Tooltips for correlation concepts
const CORRELATION_TOOLTIPS = {
  avgCorrelation: 'Average correlation between all pairs. Lower is better for diversification.',
  riskContribution: 'How much each position contributes to total portfolio risk.',
  diversificationBenefit: 'Risk reduction achieved through diversification vs holding individual stocks.',
  rollingCorrelation: 'How correlation changes over time. Spikes often occur during market stress.',
  clusters: 'Groups of stocks that move together. Hidden concentration risk if one cluster dominates.'
};
...
function CorrelationPanel({ portfolioId }) {
  ...
  // Ask AI context for correlation panel
  const askAIProps = useAskAI(() => ({
    ...
    highlyCorrelatedPairs: correlationData?.highlyCorrelated?.length || 0,
    diversificationBenefit: correlationData?.diversificationBenefit,
    clusterCount: clusterData?.clusters?.length || 0
  }));
  ...
  // Color scale for correlation values (-1 to 1) - Prism Design System
  const getCorrelationColor = (value) => {
    if (value === null || value === undefined) return 'var(--bg-tertiary)';
    if (value >= 0.8) return 'var(--negative)'; // High positive - red (risky)
    if (value >= 0.5) return 'var(--warning-dark)'; // Medium positive - amber
    if (value >= 0.2) return 'var(--warning-dark)'; // Low positive - amber
    if (value >= -0.2) return 'var(--positive)'; // Uncorrelated - green (good)
    if (value >= -0.5) return 'var(--info)'; // Low negative - cyan
    return 'var(--info)'; // Negative correlation - cyan (excellent)
  };
  ...
  const CorrelationMatrix = () => {
    ...
    return (
      <div className="correlation-matrix-section">
        ...
        {/* Color legend - Prism Design System */}
        <div className="matrix-legend">
          <span className="legend-label">Correlation:</span>
          <div className="legend-scale">
            <div className="legend-item" style={{ background: 'var(--info)' }}>-1</div>
            <div className="legend-item" style={{ background: 'var(--info)' }}>-0.5</div>
            <div className="legend-item" style={{ background: 'var(--positive)' }}>0</div>
            <div className="legend-item" style={{ background: 'var(--warning-dark)' }}>0.3</div>
            <div className="legend-item" style={{ background: 'var(--warning-dark)' }}>0.6</div>
            <div className="legend-item" style={{ background: 'var(--negative)' }}>1</div>
          </div>
      ...
      <div className="risk-contribution-section">
        ...
        <div className="risk-bars">
          ...
          {riskContributions.map((rc, i) => {
            ...
            const efficiencyColor = efficiency > 1.3 ? 'var(--negative)' :
                                    efficiency > 1.1 ? 'var(--warning)' :
                                    efficiency < 0.7 ? 'var(--positive)' : 'var(--text-secondary)';
            return (
                ...
                <div className="risk-bar-container">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: efficiency > 1.2 ? 'var(--negative)' :
                                       efficiency < 0.8 ? 'var(--positive)' : 'var(--accent-primary)'
                    }}
                  />
                </div>
  ...
  return (
    <div className="simulation-panel correlation-panel" {...askAIProps}>
      <div className="panel-header">
        <Grid3X3 size={20} className="header-icon" />
        <div className="header-text">
          <h3>Correlation & Risk Analysis</h3>
          <p className="panel-description">
            Understand how your holdings move together and identify hidden concentration risks
          </p>
        </div>
        <div className="header-controls">
          <div className="period-selector">
            <label className="period-label">Analysis Period</label>
            <select
              value={period}
            ...
            <span className="period-hint">{PERIODS.find(p => p.value === period)?.hint}</span>
          </div>
          <button className="btn-icon" onClick={loadAllData} disabled={loading} title="Refresh data">
