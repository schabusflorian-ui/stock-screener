...
import { Loader, AlertTriangle, Shield, TrendingDown, Play, RefreshCw } from 'lucide-react';
...
function StressTestPanel({ portfolioId }) {
  ...
  const getSeverityLabel = (drawdown) => {
    if (drawdown > -10) return { label: 'Minimal', color: 'var(--success-color)' };
    if (drawdown > -20) return { label: 'Moderate', color: 'var(--warning-color)' };
    if (drawdown > -35) return { label: 'Significant', color: 'var(--danger-color)' };
    return { label: 'Severe', color: '#dc2626' };
  };

  return (
    <div className="simulation-panel stress-test-panel">
