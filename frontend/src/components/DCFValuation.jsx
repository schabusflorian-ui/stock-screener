...
// Static constants moved outside component to prevent recreation (Tier 3 optimization)
...
export function DCFValuation({ symbol, currentPrice, sharesOutstanding }) {
  ...
  // Memoized validation helpers using constants (Tier 3 optimization)
  const getInputStatus = useCallback((name, value) => {
  ...
  const getValidationMessage = useCallback((name, status) => {
  ...
  // Memoized formatting functions (Tier 3 optimization)
  ...
  const formatBillions = useCallback((value) => {
    if (value === null || value === undefined) return '';
    return (value / 1e9).toFixed(2);
  }, []);

  const parseBillions = useCallback((str) => {
  ...
  // Memoized range calculations for football field (Tier 3 optimization)
  // Must be before early returns to satisfy React Hooks rules
  const { rangeMin, rangeMax, rangeSpan } = useMemo(() => {
    if (!data?.scenarios) {
      return { rangeMin: 0, rangeMax: 100, rangeSpan: 100 };
    }
    const min = data.scenarios.bear.intrinsicValuePerShare * 0.9;
    const max = data.scenarios.bull.intrinsicValuePerShare * 1.1;
    return { rangeMin: min, rangeMax: max, rangeSpan: max - min };
  }, [data?.scenarios?.bear?.intrinsicValuePerShare, data?.scenarios?.bull?.intrinsicValuePerShare]);
  ...
  return (
    <FeatureGate
      feature="dcf_valuation"
      showPreview={true}
      previewHeight="400px"
      title="DCF Valuation"
      description="Professional-grade discounted cash flow analysis with multi-stage growth modeling, sensitivity analysis, and reverse DCF calculations."
    >
    <div className="dcf-valuation">
      {/* Header */}
              ...
              {sensitivityConfig.useCustomIntervals && (
                <div className="custom-intervals-grid">
                  <div className="interval-row">
                    <span className="interval-label">Row ({VARIABLE_LABELS[sensitivityConfig.rowVariable]}):</span>
                  ...
                  <div className="interval-row">
                    <span className="interval-label">Column ({VARIABLE_LABELS[sensitivityConfig.colVariable]}):</span>
                    <div className="interval-inputs">
                      <label>
                        Min
                        <input
            ...
            <div className="sensitivity-matrix">
              <div className="matrix-header">
                <span className="matrix-title">
                  Intrinsic Value: {VARIABLE_LABELS[sensitivityData.rowVariable]} vs {VARIABLE_LABELS[sensitivityData.colVariable]}
                </span>
                  ...
                  <thead>
                    <tr>
                      <th className="corner-cell">
                        {VARIABLE_LABELS[sensitivityData.rowVariable]} ↓
                      </th>
