...
import { Search, Command, Bell, Menu, ArrowRight, X, PrismSparkle, AlertTriangle, AlertCircle, Info, Building2, Briefcase, Star, CheckCircle, ArrowUpRight, Eye } from '../icons';
...
function Header({ onOpenCommandPalette, onToggleMobileSidebar, onToggleChatPanel, isChatPanelOpen }) {
  ...
  // Get icon for notification - using Prism icons instead of emojis
  const getNotificationIcon = (notification) => {
    // Legacy alerts have signal_type
    if (notification.signal_type) {
      const icons = {
        strong_buy: { Icon: CheckCircle, color: '#059669' },
        buy: { Icon: ArrowUpRight, color: '#2563EB' },
        watch: { Icon: Eye, color: '#D97706' },
    ...
    <header className="header">
      <div className="header-left">
        <button className="mobile-menu-btn" onClick={onToggleMobileSidebar}>
          <Menu size={20} className="prism-icon--light" />
        </button>
      </div>

      <div className="header-center">
        <button className="search-trigger" onClick={onOpenCommandPalette} data-tour="search">
          <Search size={16} className="search-icon prism-icon--light" />
      ...
      <div className="header-right" ref={dropdownRef}>
        ...
        <button
          ...
          data-tour="ai-chat"
        >
          <PrismSparkle size={18} className="prism-icon--light" />
        </button>
        ...
        >
          <Bell size={18} className="prism-icon--light" />
          {unreadCount > 0 && (
