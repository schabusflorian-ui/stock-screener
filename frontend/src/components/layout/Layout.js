...
// Lazy load heavy modals - only loads when user opens them
const ChatPanel = lazy(() => import('../nl/ChatPanel'));
...
function Layout({ children }) {
  ...
  return (
    <div className={`app-layout ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${isPanelOpen ? 'chat-panel-open' : ''}`}>
      ...
      {/* AI Chat Panel - lazy loaded only when opened */}
      {isPanelOpen && (
        <Suspense fallback={<div className="chat-panel-loading" />}>
          <ChatPanel />
        </Suspense>
      )}
    </div>
