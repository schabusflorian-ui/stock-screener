...
function CommandPalette({ open, onOpenChange }) {
  ...
  const { openPanel, addMessage, setContext, clearConversation } = useNLQuery();
      ...
      <div className="command-palette-container" onClick={e => e.stopPropagation()}>
        <Command className="command-palette" shouldFilter={false}>
          <div className="command-input-wrapper">
            {mode === 'ask' ? (
              <PrismSparkle size={18} className="command-search-icon ai" />
            ) : (
              ...
              <div className="nl-response-container">
                {nlLoading ? (
                  <div className="nl-loading">
                    <PrismSparkle size={20} className="nl-loading-icon" />
                  ...
                  <div className="nl-error">
                    <AlertTriangle size={14} className="nl-error-icon" />
                  ...
                  <div className="nl-response">
                    <div className="nl-response-header">
                      <PrismSparkle size={16} className="nl-response-icon" />
                    ...
                    <div className="nl-response-actions">
                      ...
                      <button
                        ...
                        onClick={() => {
                          // Close command palette and open the sidebar chat panel
                          onOpenChange(false);
                          // Clear previous conversation and set up context
                          clearConversation();
                          setContext({
                            symbol: nlResponse.company,
                            previousQuery: nlResponse.query
                          });
                          // Add the previous Q&A to the conversation
                          addMessage({ role: 'user', content: nlResponse.query });
                          addMessage({ role: 'assistant', content: nlResponse.response });
              ...
              <Command.Group heading={`Ask about ${currentCompany}`} className="command-group company-queries">
                {loadingContext ? (
                  ...
                  companyQueries.map((item, idx) => (
                    ...
                    >
                      <PrismSparkle size={16} className="command-item-icon ai-icon" />
                      <div className="command-item-content">
                        <span className="command-item-label">{item.label}</span>
                      </div>
                      <PrismSparkle size={14} className="command-item-arrow ai" />
                    </Command.Item>
                  ))
                )}
              </Command.Group>
