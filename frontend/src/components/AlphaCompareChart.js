...
import { Activity, RefreshCcw } from './icons';
...
const COMPANY_COLORS = ['#2563EB', '#059669', '#D97706', '#DC2626', '#7C3AED', '#0891B2', '#DB2777'];

// Prism chart configuration
const PRISM_CHART_CONFIG = {
  grid: {
    stroke: '#F1F5F9',
    strokeDasharray: '0', // solid lines per Prism spec
  },
  axis: {
...
export function AlphaCompareChart({ symbols = [], height = 350, showControls = true }) {
  ...
  const askAIProps = useAskAI(createChartExtractor(() => ({
    symbol: symbols?.join(', ') || '',
    metric: 'alpha_comparison',
    period,
    companyName: `Alpha comparison: ${symbols?.join(' vs ') || ''}`
  })));
  ...
  return (
    <div className="alpha-compare-container" {...askAIProps}>
      {/* Summary Cards */}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            <CartesianGrid
              ...
              stroke={PRISM_CHART_CONFIG.grid.stroke}
              vertical={false}
            />
            <XAxis
              dataKey="date"
              tickFormatter={formatDate}
              tick={{ fontSize: PRISM_CHART_CONFIG.axis.fontSize, fill: PRISM_CHART_CONFIG.axis.textColor, fontFamily: PRISM_CHART_CONFIG.axis.fontFamily }}
              tickLine={false}
              axisLine={{ stroke: PRISM_CHART_CONFIG.grid.stroke }}
