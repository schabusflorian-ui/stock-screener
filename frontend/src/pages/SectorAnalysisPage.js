...
import { Columns, X, TrendingUp, BarChart2, Building2, Tag, RefreshCw, Crown, Percent, LineChart, LayoutGrid, List } from '../components/icons';
...
// Row components with Ask AI support
const ConstituentRow = ({ company }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - ${company.name}`,
    data: { sector: company.sector, industry: company.industry, marketCap: company.market_cap }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
...
const TopPerformerRow = ({ company, idx, metricKey, fmt }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Top Performer`,
    data: { rank: idx + 1, metric: metricKey, value: company[metricKey] }
  }));
  return (
    <tr {...askAIProps}>
      ...
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
      <td className="company-name">{company.name}</td>
      <td className="metric-value positive">{fmt.percent(company[metricKey] / 100)}</td>
...
const SectorCard = ({ sector, children }) => {
  ...
  return (
    <div className="sector-card" {...askAIProps}>
      {children}
    </div>
  );
};

const PerformerRow = ({ company, idx, sector, topMetric, getValueClass, formatValue }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Top Performer in ${sector}`,
    data: { rank: idx + 1, metric: topMetric, value: company[topMetric] }
  }));
  return (
    <div className="performer-row" {...askAIProps}>
...
const IndustryCompanyRow = ({ company, industry, getValueClass, formatValue }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - ${company.name}`,
    data: { industry, roic: company.roic, roe: company.roe, grossMargin: company.gross_margin }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
      ...
      <td className={getValueClass(company.roic, { good: 15, bad: 5 })}>{formatValue(company.roic, 'percent')}</td>
      <td className={getValueClass(company.roe, { good: 15, bad: 5 })}>{formatValue(company.roe, 'percent')}</td>
      <td className={getValueClass(company.gross_margin, { good: 40, bad: 20 })}>{formatValue(company.gross_margin, 'percent')}</td>
      <td className={getValueClass(company.operating_margin, { good: 20, bad: 5 })}>{formatValue(company.operating_margin, 'percent')}</td>
      <td className={getValueClass(company.net_margin, { good: 15, bad: 0 })}>{formatValue(company.net_margin, 'percent')}</td>
      <td>{formatValue(company.pe_ratio, 'ratio')}</td>
      <td className={getValueClass(company.revenue_growth, { good: 10, bad: 0 })}>{formatValue(company.revenue_growth, 'percent')}</td>
      <td><WatchlistButton symbol={company.symbol} name={company.name} size="small" /></td>
    </tr>
  );
};

const SectorCompanyRow = ({ company, sector, getValueClass, formatValue }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - ${company.name}`,
    data: { sector, roic: company.roic, netMargin: company.net_margin, peRatio: company.pe_ratio }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
      ...
      <td className={getValueClass(company.net_margin, { good: 15, bad: 0 })}>{formatValue(company.net_margin, 'percent')}</td>
      <td>{formatValue(company.pe_ratio, 'ratio')}</td>
      <td><WatchlistButton symbol={company.symbol} name={company.name} sector={sector} size="small" /></td>
    </tr>
  );
};
...
// Tab definitions - using Prism icons
const TABS = [
  { id: 'indices', label: 'Market Indices', Icon: TrendingUp },
  { id: 'overview', label: 'Sector Overview', Icon: BarChart2 },
  { id: 'industries', label: 'Industries', Icon: Building2 },
  { id: 'custom-sectors', label: 'Custom Sectors', Icon: Tag },
  { id: 'rotation', label: 'Sector Rotation', Icon: RefreshCw },
  { id: 'top-performers', label: 'Top Performers', Icon: Crown },
  { id: 'margins', label: 'Margin Analysis', Icon: Percent }
];
...
// Profitability metrics show both median and average (median is more robust to outliers)
const SECTOR_OVERVIEW_METRICS = [
  { key: 'company_count', label: 'Companies', format: 'integer' },
  { key: 'median_roic', label: 'ROIC', format: 'percent_dual', medianKey: 'median_roic', avgKey: 'avg_roic', thresholds: { good: 15, bad: 5 } },
  { key: 'median_roe', label: 'ROE', format: 'percent_dual', medianKey: 'median_roe', avgKey: 'avg_roe', thresholds: { good: 15, bad: 5 } },
  ...
  { key: 'median_net_margin', label: 'Net Margin', format: 'percent_dual', medianKey: 'median_net_margin', avgKey: 'avg_net_margin', thresholds: { good: 15, bad: 0 } },
  { key: 'avg_fcf_margin', label: 'FCF Margin', format: 'percent', thresholds: { good: 15, bad: 0 } },
  { key: 'median_pe_ratio', label: 'P/E', format: 'ratio_dual', medianKey: 'median_pe_ratio', avgKey: 'avg_pe_ratio' },
  ...
  { key: 'median_revenue_growth', label: 'Growth', format: 'percent_dual', medianKey: 'median_revenue_growth', avgKey: 'avg_revenue_growth', thresholds: { good: 10, bad: 0 } },
...
// View modes for sector overview - using Prism icons
const VIEW_MODES = [
...
function SectorAnalysisPage() {
  ...
  // Using median_* keys for profitability metrics (more robust to outliers)
  const [selectedSectorMetrics, setSelectedSectorMetrics] = useState(['company_count', 'median_roic', 'median_net_margin', 'median_pe_ratio', 'total_market_cap_b']);
  ...
  // Handles percent_dual and ratio_dual formats for showing median/avg
  const sectorTableColumns = useMemo(() => {
    ...
    const metricColumns = selectedSectorMetrics.map(metricKey => {
      ...
      // Handle dual format (median / avg) rendering
      if (metric.format === 'percent_dual' || metric.format === 'ratio_dual') {
        const isPercent = metric.format === 'percent_dual';
        return {
          ...metric,
          format: isPercent ? 'percent' : 'ratio',
          render: (row) => {
            const medianVal = row[metric.medianKey];
            const avgVal = row[metric.avgKey];
            const formatFn = (v) => v == null ? '-' : (isPercent ? `${v.toFixed(1)}%` : v.toFixed(2));
            // Show median as primary value with avg in parentheses
            const medianFormatted = formatFn(medianVal);
            const avgFormatted = formatFn(avgVal);
            // Get value class based on median value
            const valueClass = medianVal != null && metric.thresholds
              ? (metric.thresholds.good !== undefined && medianVal >= metric.thresholds.good ? 'positive'
                : metric.thresholds.bad !== undefined && medianVal <= metric.thresholds.bad ? 'negative' : '')
              : '';
            return (
              <span className={valueClass} title={`Median: ${medianFormatted}, Avg: ${avgFormatted}`}>
                {medianFormatted} <span className="secondary-value">({avgFormatted})</span>
              </span>
            );
          }
        };
              ...
              <div className="index-chart-section">
                ...
                {indexPriceHistory[selectedIndex.symbol]?.length > 0 ? (
                  <div className="index-chart-container">
                    <AskAIProvider value={{ type: 'chart', label: `${selectedIndex.short_name} Index`, metric: 'index_price', symbol: selectedIndex.symbol }}>
                      <MultiMetricChart
                        ...
                        <table className="constituents-table">
                          ...
                          <tbody>
                            {paginatedConstituents.map(company => (
                              <ConstituentRow key={company.symbol} company={company} />
                            ))}
                          </tbody>
                        </table>
                      </div>
                    ...
                    >
                      ...
                      <div className="card-metrics">
                        <div className="metric">
                          ...
                          <span className={`value ${getValueClass(sector.median_roic, { good: 15, bad: 5 })}`}>
                            {formatValue(sector.median_roic, 'percent')}
                          </span>
                        </div>
                        <div className="metric">
                          <span className="label">Net Margin</span>
                          <span className={`value ${getValueClass(sector.median_net_margin, { good: 15, bad: 0 })}`}>
                            {formatValue(sector.median_net_margin, 'percent')}
                          </span>
                        </div>
                        <div className="metric">
                          <span className="label">P/E</span>
                          <span className="value">{formatValue(sector.median_pe_ratio, 'ratio')}</span>
                        </div>
                        <div className="metric">
                          <span className="label">D/E</span>
              ...
              {viewMode === 'chart' && (
                <div className="sector-charts">
                  <div className="charts-grid">
                    <AskAIProvider value={{ type: 'chart', label: 'Average ROIC by Sector', metric: 'sector_roic' }}>
                      <MetricsBarChart
                    ...
                    <AskAIProvider value={{ type: 'chart', label: 'Average Net Margin by Sector', metric: 'sector_net_margin' }}>
                      <MetricsBarChart
                    ...
                    <AskAIProvider value={{ type: 'chart', label: 'Average Revenue Growth by Sector', metric: 'sector_revenue_growth' }}>
                      <MetricsBarChart
                    ...
                    <AskAIProvider value={{ type: 'chart', label: 'Total Market Cap by Sector', metric: 'sector_market_cap' }}>
                      <MetricsBarChart
            ...
            <div className="custom-companies">
              ...
              {!selectedCustomSector && !selectedTagFilter && (
                <div className="select-prompt">
                  <div className="prompt-icon"><Tag size={32} /></div>
      ...
      {activeTab === 'rotation' && (
        <div className="tab-content">
          ...
          <div className="rotation-chart-container">
            <AskAIProvider value={{ type: 'chart', label: 'Sector ROIC Over Time', metric: 'sector_rotation' }}>
              <MultiMetricChart
    ...
    <div className="industry-detail">
      ...
      {companies.length > 0 && (
        <div className="industry-charts">
          <AskAIProvider value={{ type: 'chart', label: `Top Companies by ROIC in ${industry}`, metric: 'industry_roic_ranking', industry }}>
            <MetricsBarChart
              data={companies
