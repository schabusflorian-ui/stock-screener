...
import api, { unifiedStrategyAPI } from '../../services/api';
            ...
            <>
              ...
              {strategyDetails ? (
                <div className="details-content">
                  ...
                  {/* ML Model Version - for ML-enabled strategies */}
                  {(strategyDetails.mlModelVersion || strategyDetails.useMLCombiner) && (
                    <div className="detail-section">
                      <h4>ML Model</h4>
                      <div className="params-grid">
                        <div className="param">
                          <span>Model Version</span>
                          <strong>{strategyDetails.mlModelVersion || 'Not set'}</strong>
                        </div>
                        <div className="param">
                          <span>Version Lock</span>
                          <strong className={strategyDetails.mlModelLocked ? 'locked' : 'unlocked'}>
                            {strategyDetails.mlModelLocked ? <><Lock size={14} /> Locked</> : <><Unlock size={14} /> Auto-update</>}
                          </strong>
                        </div>
                        {strategyDetails.mlModelUpdatedAt && (
                          <div className="param">
                            <span>Last Updated</span>
                            <strong>{new Date(strategyDetails.mlModelUpdatedAt).toLocaleDateString()}</strong>
                          </div>
                        )}
                      </div>
                      <p className="ml-model-note">
                        {strategyDetails.mlModelLocked
                          ? 'Model version is locked. Updates will not apply automatically.'
                          : 'Model version will update automatically when new models are promoted.'}
                      </p>
                    </div>
                  )}
                </div>
