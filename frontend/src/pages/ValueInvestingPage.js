...
const REGIME_CONFIG = {
  CRISIS: { color: '#dc2626', icon: AlertTriangle, label: 'Crisis Mode' },
  LATE_CYCLE: { color: '#f59e0b', icon: Activity, label: 'Late Cycle' },
  FEAR: { color: '#f97316', icon: TrendingDown, label: 'Fear Mode' },
  EARLY_CYCLE: { color: '#22c55e', icon: TrendingUp, label: 'Early Cycle' },
  NEUTRAL: { color: '#3b82f6', icon: BarChart3, label: 'Neutral' }
};
...
function MacroCard({ title, value, subtext, status, icon: Icon }) {
  ...
  return (
    <div className="macro-card" style={{ borderLeftColor: statusColors[status] || statusColors.neutral }}>
...
function RegimeBanner({ regime, macroContext }) {
  ...
  return (
    <div className="regime-banner" style={{ backgroundColor: `${config.color}15`, borderColor: config.color }}>
