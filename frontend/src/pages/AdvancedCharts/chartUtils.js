...
export function alignSeries(series1, series2) {
  ...
  return {
    aligned1: commonPeriods.map(p => map1.get(p)).filter(v => v !== null && !isNaN(v)),
    aligned2: commonPeriods.map(p => map2.get(p)).filter(v => v !== null && !isNaN(v)),
