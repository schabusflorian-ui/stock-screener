...
const IPO_STAGES = {
  S1_FILED: { name: 'Filed', shortName: 'Filed', className: 'stage-filed', order: 1 },
  S1_AMENDED: { name: 'Amended', shortName: 'Amended', className: 'stage-amended', order: 2 },
  PRICE_RANGE_SET: { name: 'Price Set', shortName: 'Price Set', className: 'stage-price-set', order: 3 },
  EFFECTIVE: { name: 'Effective', shortName: 'Effective', className: 'stage-effective', order: 4 },
  PRICED: { name: 'Priced', shortName: 'Priced', className: 'stage-priced', order: 5 },
  TRADING: { name: 'Trading', shortName: 'Trading', className: 'stage-trading', order: 6 }
};
...
function IPOCard({ ipo, formatCurrency, formatDate }) {
  ...
  return (
    <div className={`ipo-card ${isTrading ? 'trading' : ''}`}>
      ...
      {isTrading && tradingTicker && ipo.company_id && (
        <Link to={`/company/${tradingTicker}`} className="trading-link-btn">
          View Stock Analysis
        </Link>
      )}
      {isTrading && tradingTicker && !ipo.company_id && (
        <span className="trading-link-btn disabled" title="Company data not yet available">
          Coming Soon
        </span>
      )}
    </div>
...
function IPOPipelinePage() {
  ...
  // Sync trading companies
  const syncTradingCompanies = async () => {
    setSyncing(true);
    setSyncResult(null);
    try {
      const res = await ipoAPI.syncTradingCompanies();
  ...
  return (
    <div className="ipo-pipeline-page">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button
              variant="secondary"
              onClick={syncTradingCompanies}
              disabled={syncing}
              title="Create company records for trading IPOs"
            >
      ...
      {/* Sync Result Banner */}
      {syncResult && (
        <div className={`check-result ${syncResult.error ? 'error' : 'success'}`}>
          {syncResult.error ? (
            <span>Error: {syncResult.error}</span>
          ) : (
            <span>
              Synced {syncResult.created || 0} new companies, linked {syncResult.linked || 0} existing
      ...
      {/* Quick Filter Tabs - Above Kanban/List */}
      {!searchResults && (
        <div className="ipo-filter-tabs">
          <button
          ...
          <button
            className={`filter-tab ${filterType === 'watchlist' ? 'active' : ''}`}
              ...
              {Object.entries(IPO_STAGES).map(([status, config]) => {
                ...
                return (
                  <div key={status} className="kanban-column">
                    <div className={`column-header ${config.className}`}>
                  ...
                  {listData.map(ipo => {
                    ...
                    return (
                      <tr key={ipo.id} className={isTrading ? 'trading-row' : ''}>
                        ...
                        <td>
                          <span className={`status-badge ${stage?.className || 'stage-filed'}`}>
                        ...
                        <td className="actions-cell">
                          ...
                          {isTrading && tradingTicker && !ipo.company_id && (
                            <span className="view-stock-link disabled" title="Company data not yet available">
                              Coming Soon
                            </span>
                          )}
                        </td>
