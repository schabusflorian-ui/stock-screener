...
// Table row components with Ask AI support
const ShareholderReturnRow = ({ company, formatCurrency, formatPercent }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Shareholder Returns`,
    data: {
      totalReturn: company.total_shareholder_return,
      dividends: company.dividends_paid,
      buybacks: company.buybacks_executed
    }
  }));
  return (
    <tr {...askAIProps}>
...
const DividendYielderRow = ({ company }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Dividend Yield`,
    data: { yield: company.dividend_yield, growth5y: company.dividend_growth_5y, yearsGrowth: company.years_of_growth }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
...
const DividendGrowthRow = ({ company }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Dividend Growth`,
    data: { growthRate: company.growth_rate, yield: company.dividend_yield }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
...
const AristocratRow = ({ company }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Dividend Aristocrat`,
    data: { yearsGrowth: company.years_of_growth, yield: company.dividend_yield, growth5y: company.dividend_growth_5y }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
...
const BuybackRow = ({ company }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: company.symbol,
    label: `${company.symbol} - Buybacks`,
    data: { buybacks: company.buybacks_executed, dividends: company.dividends_paid, totalReturn: company.total_shareholder_return }
  }));
  return (
    <tr {...askAIProps}>
      <td><Link to={`/company/${company.symbol}`} className="symbol-link">{company.symbol}</Link></td>
...
const EventRow = ({ event }) => {
  const askAIProps = useAskAI(() => ({
    type: 'table_row',
    symbol: event.symbol,
    label: `${event.symbol} - ${event.event_type}`,
    data: { eventType: event.event_type, date: event.event_date, headline: event.headline }
  }));
  return (
    <tr {...askAIProps}>
      <td>{formatDate(event.event_date)}</td>
      <td><Link to={`/company/${event.symbol}`} className="symbol-link">{event.symbol}</Link></td>
    ...
    <div className="overview-content">
      ...
      {stats && (
        <div className="stats-grid">
          ...
          <AskAIProvider value={{ type: 'metric', metric: 'avg_dividend_yield', label: 'Average Dividend Yield', value: stats.dividends?.avg_yield }}>
            <div className="stat-card" data-ask-ai="true">
          ...
          <AskAIProvider value={{ type: 'metric', metric: 'total_buybacks', label: 'Total Buybacks', value: stats.buybacks?.total_spent }}>
            <div className="stat-card highlight" data-ask-ai="true">
      ...
      {dividendsBySector.length > 0 && (
        <AskAIProvider value={{ type: 'chart', metric: 'dividend_yield_by_sector', label: 'Dividend Yield by Sector' }}>
          <div className="card chart-card" data-ask-ai="true">
          ...
          <tbody>
            {sortedYielders.length === 0 ? (
              ...
              sortedYielders.map((company) => (
                <DividendYielderRow key={company.id || company.symbol} company={company} />
              ))
            )}
          </tbody>
        </table>
          ...
          <tbody>
            {sortedGrowthLeaders.length === 0 ? (
              ...
              sortedGrowthLeaders.map((company) => (
                <DividendGrowthRow key={company.id || company.symbol} company={company} />
              ))
            )}
          </tbody>
        </table>
          ...
          <tbody>
            {sortedAristocrats.length === 0 ? (
              ...
              sortedAristocrats.map((company) => (
                <AristocratRow key={company.id || company.symbol} company={company} />
              ))
            )}
          </tbody>
        </table>
            ...
            <tbody>
              {sortedBuybacks.length === 0 ? (
                ...
                sortedBuybacks.map((company) => (
                  <BuybackRow key={company.id || company.symbol} company={company} />
                ))
              )}
            </tbody>
          </table>
