...
function AgentSettingsPage() {
  ...
  const loadAgent = useCallback(async () => {
    try {
      ...
      const mergedSettings = {
        ...
        // Regime
        regime_scaling_enabled: agentData.regime_scaling_enabled ?? DEFAULT_SETTINGS.regime_scaling_enabled,
        vix_scaling_enabled: agentData.vix_scaling_enabled ?? DEFAULT_SETTINGS.vix_scaling_enabled,
        vix_threshold: agentData.vix_threshold ?? DEFAULT_SETTINGS.vix_threshold,
        pause_in_crisis: agentData.pause_in_crisis ?? DEFAULT_SETTINGS.pause_in_crisis,
        // Execution
        auto_execute: agentData.auto_execute ?? DEFAULT_SETTINGS.auto_execute,
        execution_threshold: agentData.execution_threshold ?? DEFAULT_SETTINGS.execution_threshold,
        require_confirmation: agentData.require_confirmation ?? DEFAULT_SETTINGS.require_confirmation,
        allowed_actions: agentData.allowed_actions ?? DEFAULT_SETTINGS.allowed_actions,
        // Features
        use_optimized_weights: agentData.use_optimized_weights ?? DEFAULT_SETTINGS.use_optimized_weights,
        use_hmm_regime: agentData.use_hmm_regime ?? DEFAULT_SETTINGS.use_hmm_regime,
        use_ml_combiner: agentData.use_ml_combiner ?? DEFAULT_SETTINGS.use_ml_combiner,
        use_factor_exposure: agentData.use_factor_exposure ?? DEFAULT_SETTINGS.use_factor_exposure,
        use_probabilistic_dcf: agentData.use_probabilistic_dcf ?? DEFAULT_SETTINGS.use_probabilistic_dcf,
        // Universe
  ...
  const handleActionToggle = (action) => {
    setSettings(prev => {
      const currentActions = prev.allowed_actions || ['buy', 'sell'];
                ...
                <div className={`execution-settings ${!settings.auto_execute ? 'disabled' : ''}`}>
                  ...
                  <div className="action-toggles">
                    ...
                    <div className="action-toggles__buttons">
                      <button
                        className={`action-toggle ${settings.allowed_actions?.includes('buy') ? 'active' : ''}`}
                      ...
                      <button
                        className={`action-toggle ${settings.allowed_actions?.includes('sell') ? 'active' : ''}`}
