...
import { companyAPI, trendsAPI, insidersAPI, sentimentAPI, capitalAPI, pricesAPI, dividendsAPI, indicesAPI, investorsAPI, analystAPI, aiRatingsAPI, congressionalAPI, prismAPI } from '../services/api';
import { PRISMReport } from '../components/prism';
import {
  PeriodToggle,
  MetricSelector,
  FinancialBreakdown,
...
// MultiLensAnalysis moved to separate section
// import { MultiLensAnalysis } from '../components/company';
...
const getMetricRating = (value, metricKey) => {
  ...
  // Backend stores roic, roe, margins as whole percentages (not decimals)
  const thresholds = {
  ...
  // All percentage metrics from backend are already in percentage form (e.g., 26.5 = 26.5%)
...
// MetricBar component with visual progress bar (memoized for performance)
// format: 'percent' (value already as %), 'decimal_percent' (same as percent - kept for compatibility), 'ratio' (no % suffix)
// Note: Backend stores all percentage metrics as whole numbers (26.5 = 26.5%), NOT decimals
const MetricBar = React.memo(({ label, value, max, format = 'percent', inverted = false }) => {
  const displayValue = value === null || value === undefined ? null : value;
  // Backend stores percentages as whole numbers (e.g., 26.5 = 26.5%), so no multiplication needed
  const isPercent = format === 'percent' || format === 'decimal_percent';
  const formattedValue = displayValue !== null
    ? (isPercent ? `${displayValue.toFixed(1)}%` : displayValue.toFixed(2))
    : '-';

  // Calculate bar width (0-100%)
  const normalizedValue = displayValue !== null
    ? Math.min(Math.max(displayValue, 0), max)
    : 0;
...
function CompanyPage() {
  ...
  const formatValue = (value, format) => {
    ...
    switch (format) {
      case 'decimal_percent': // Metrics stored as percentages (e.g., 26.5 = 26.5%)
        // Note: Backend stores roic, roe, margins as whole percentages (not decimals)
  ...
  const [dcfBlocked, setDcfBlocked] = useState(false);
  ...
  const [sentimentView, setSentimentView] = useState('combined'); // 'combined', 'reddit', 'stocktwits', 'news', 'analyst', 'ai_signals'
  const [insiderTab, setInsiderTab] = useState('corporate'); // 'corporate', 'congressional'
  ...
  const loadMetrics = useCallback(async () => {
    try {
      ...
      if (!periodAutoSelected && metricsRes.data.available_periods) {
        ...
        // Prefer annual data for cleaner long-term view
        // Fall back to quarterly if no annual data exists
        if (annualCount > 0) {
          setPeriodType('annual');
        } else if (quarterlyCount > 0) {
  ...
  useEffect(() => {
    const loadCompanyData = async () => {
      try {
        // Load company data first - this is required
        const companyRes = await companyAPI.getOne(symbol);
        setCompany(companyRes.data);
        setLoading(false);

        // Load trends separately - failure shouldn't block the page
        try {
          const trendsRes = await trendsAPI.getCompanyTrend(symbol);
  ...
  // Multi-Lens Analysis data loading moved to separate component
  // useEffect(() => {
  //   const loadMultiLensData = async () => {
  //     if (!symbol) return;
  //     setFactorLoading(true);
  //     try {
  //       const analysisRes = await companyAPI.getAnalysis(symbol).catch(() => null);
  //       if (analysisRes?.data) {
  //         setAnalysisData(analysisRes.data);
  //       }
  //     } catch (error) {
  //       console.error('Error loading multi-lens data:', error);
  //     } finally {
  ...
  return (
    <div className="company-page">
      ...
      <header className="company-header-new">
        ...
        {/* Right side: Stock + Quality + Actions - with visual dividers */}
        <div className="header-metrics-row">
          ...
          {/* Quality Section - Expanded with full legend - Clickable */}
          {latestMetrics?.data_quality_score !== undefined && (
            <>
              <div className="header-section-divider" />
              <button
                className="header-metric-box quality-box-wide quality-clickable"
                onClick={() => { setMainTab('analysis'); setAnalysisSection('quality'); }}
              ...
              <div className="quality-content-wide">
                <span className={`quality-number-large ${
                  latestMetrics.data_quality_score >= 70 ? 'excellent' :
                  latestMetrics.data_quality_score >= 50 ? 'good' :
                  latestMetrics.data_quality_score >= 30 ? 'fair' : 'poor'
                }`}>{Math.round(latestMetrics.data_quality_score)}</span>
                <div className="quality-legend">
                  {[
                    // Backend stores percentages as whole numbers (26 = 26%)
                    { key: 'Value', cssClass: 'value', value: latestMetrics.roic || 0 },
                    { key: 'Growth', cssClass: 'growth', value: latestMetrics.revenue_growth_yoy || 0 },
                    { key: 'Health', cssClass: 'health', value: latestMetrics.current_ratio ? Math.min(latestMetrics.current_ratio * 33, 100) : 0 },
                    { key: 'Profit', cssClass: 'profit', value: latestMetrics.net_margin || 0 }
                  ].map((dim) => {
                    const val = Math.min(Math.max(dim.value, 0), 100);
                    return (
                      <div key={dim.key} className="quality-legend-item">
                        <span className={`quality-legend-label ${dim.cssClass}`}>{dim.key}</span>
                        <div className="quality-legend-bar-bg">
                          <div className={`quality-legend-bar-fg ${dim.cssClass}`} style={{ width: `${val}%` }} />
                        </div>
                      </div>
                    );
                  })}
      ...
      <nav className="main-tabs-new">
        {['overview', 'analysis', 'financials', 'stock', 'history', 'notes', 'news', 'prism'].map(tab => (
          <button
            key={tab}
            className={`${mainTab === tab ? 'active' : ''} ${tab === 'prism' ? 'prism-tab' : ''}`}
            onClick={() => setMainTab(tab)}
          >
            {tab === 'overview' && <BarChart3 size={16} />}
            {tab === 'prism' && <PrismLogo size={16} />}
            ...
            <span>{tab === 'stock' ? 'Stock & Capital' : tab === 'prism' ? 'PRISM Report' : tab.charAt(0).toUpperCase() + tab.slice(1)}</span>
          </button>
        ))}
      </nav>
        ...
        <div className="overview-v3">
          ...
          <section className="metrics-dashboard-v4">
            ...
            <div className="hero-metrics">
              ...
              {dcfBlocked ? (
                <div className="hero-metric dcf hero-metric--locked">
                  <div className="hero-icon">
                    <Target size={18} />
                  </div>
                  <div className="hero-content">
                    <span className="hero-value locked">
                      <LockedIndicator feature="dcf_valuation" variant="badge" size="small" />
                    </span>
                    <span className="hero-label">DCF Fair Value</span>
                  </div>
                </div>
              ) : dcfData && (() => {
          ...
          <div className="bottom-panels-row">
              ...
              <div className="panel-title-row">
                ...
                <div className="sentiment-tabs">
                  {[
                    ...
                    <button
                      key={key}
                      className={`sentiment-tab ${sentimentView === key ? 'active' : ''}`}
                      onClick={() => setSentimentView(key)}
                    >
                      {label}
                    </button>
                  ))}
            ...
            {/* Right Column: Insider Trading + Famous Investors */}
            <div className="right-panels-stack">
              {/* Combined Insider & Congressional Trading Panel */}
              <section className="insider-panel-v3">
                <div className="panel-title-row">
                  <h3>
                    <Users size={14} />
                    Insider Trading (6 months)
                  </h3>
                  <div className="insider-tabs">
                    <button
                      className={`insider-tab ${insiderTab === 'corporate' ? 'active' : ''}`}
                      onClick={() => setInsiderTab('corporate')}
                    >
                      Corporate
                    </button>
                    <button
                      className={`insider-tab ${insiderTab === 'congressional' ? 'active' : ''}`}
                      onClick={() => setInsiderTab('congressional')}
                    >
                      Congress
                    </button>
                  </div>
                ...
                {/* Corporate Insiders Tab */}
                {insiderTab === 'corporate' && (
                  <>
                    {insiderData && insiderData.summaries?.['6m'] ? (
                      <div className="insider-content-v3">
                        ...
                        {insiderData.transactions && insiderData.transactions.length > 0 && (
                          <div className="insider-list-v3">
                            {insiderData.transactions.slice(0, 5).map((tx, i) => (
                              <div key={i} className="tx-row-v3">
                                ...
                                <span className="tx-shares-v3">{((tx.shares_transacted || tx.shares || 0) / 1000).toFixed(0)}K</span>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <div className="no-insider-v3">
                        <Users size={24} />
                        <p>No corporate insider activity</p>
                      </div>
                    )}
                  </>
                )}

                {/* Congressional Trading Tab */}
                {insiderTab === 'congressional' && (
                  <>
                    {congressionalLoading ? (
                      ...
                      <div className="insider-content-v3">
                        <div className="insider-header-v3">
                          <div className="insider-stats-v3 congressional">
                            <div className="stat">
                              <span className="num">{congressionalData.trades.length}</span>
                              <span className="lbl">Trades</span>
                            </div>
                            <div className="stat">
                              <span className="num">{new Set(congressionalData.trades.map(t => t.politician_id)).size}</span>
                              <span className="lbl">Members</span>
                            </div>
                            <div className="stat buy">
                              <span className="num">{congressionalData.trades.filter(t => t.transaction_type === 'purchase').length}</span>
                              <span className="lbl">Buys</span>
                            </div>
                          </div>
                        </div>
                        <div className="insider-list-v3">
                          {congressionalData.trades.slice(0, 5).map((tx, i) => (
                            <div key={i} className="tx-row-v3">
                              <span className={`tx-type-v3 ${tx.transaction_type}`}>
                                {tx.transaction_type === 'purchase' ? 'BUY' : 'SELL'}
                              </span>
                              <span className="tx-name-v3" title={tx.politician_name}>
                                {tx.politician_name?.split(' ').slice(-1)[0]}
                              </span>
                              <span className="tx-party-v3">
                                {tx.chamber === 'Senate' ? 'S' : 'H'}-{tx.party}
                              </span>
                            </div>
                          ))}
                        </div>
                        <Link to="/signals?tab=congressional" className="view-more-link">
                      ...
                      <div className="no-insider-v3">
                        <Building2 size={24} />
                        <p>No congressional trading data</p>
                      </div>
                    )}
                  </>
                )}
              ...
              {investorOwners.length > 0 && (
                <section className="famous-investors-panel">
                  <div className="panel-header">
                    ...
                    <span className="investor-count">{investorOwners.length}</span>
                  </div>
                  <div className="famous-investors-list">
                    {investorOwners.slice(0, 5).map((inv) => (
                      <Link
                      ...
                      >
                        ...
                        <span className="investor-stats">
                          <span className="weight">{inv.portfolio_weight?.toFixed(1)}%</span>
      ...
      {mainTab === 'prism' && (
        <div className="prism-tab-content">
          <PRISMReport
            symbol={symbol}
            companyName={company?.company?.name}
            currentPrice={latestMetrics?.current_price || priceData?.current?.close}
            onNavigateToDCF={() => {
              setAnalysisSection('dcf');
              setMainTab('analysis');
            }}
          />
        </div>
