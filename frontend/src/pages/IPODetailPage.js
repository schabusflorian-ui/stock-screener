...
// IPO stage definitions with CSS classes (Prism Design System compliant)
const IPO_STAGES = {
  S1_FILED: { name: 'Registration Filed', description: 'Initial S-1 filed with SEC', className: 'stage-filed', order: 1 },
  S1_AMENDED: { name: 'Amendment Filed', description: 'Responding to SEC comments', className: 'stage-amended', order: 2 },
  PRICE_RANGE_SET: { name: 'Price Range Set', description: 'Expected price range disclosed', className: 'stage-price-set', order: 3 },
  EFFECTIVE: { name: 'Registration Effective', description: 'SEC approved, ready to price', className: 'stage-effective', order: 4 },
  PRICED: { name: 'IPO Priced', description: 'Final price set, trading imminent', className: 'stage-priced', order: 5 },
  TRADING: { name: 'Now Trading', description: 'Listed and trading on exchange', className: 'stage-trading', order: 6 },
  WITHDRAWN: { name: 'Withdrawn', description: 'IPO cancelled', className: 'stage-withdrawn', order: -1 }
};
    ...
    <div className="timeline-progress">
      {stages.map((stage, index) => {
        ...
        return (
          ...
          >
            <div className={`step-dot ${stageInfo.className} ${isComplete || isCurrent ? 'active' : 'inactive'}`} />
            <div className="step-label">{stageInfo.name}</div>
            {index < stages.length - 1 && (
              <div className={`step-line ${isComplete ? stageInfo.className : 'inactive'}`} />
            )}
          </div>
        );
      })}
...
function IPODetailPage() {
  ...
  const currentStage = IPO_STAGES[ipo.status] || { name: ipo.status, className: 'stage-filed' };
      ...
      <div className="ipo-detail-grid">
        ...
        <section className="detail-card company-info">
          ...
          <div className="company-links">
            ...
            {ipo.website && (
              <a href={ensureHttps(ipo.website)} target="_blank" rel="noreferrer" className="company-link website-link">
                Company Website
              </a>
            )}
            {ipo.status === 'TRADING' && (ipo.ticker_final || ipo.ticker_proposed) && ipo.company_id && (
              <Link to={`/company/${ipo.ticker_final || ipo.ticker_proposed}`} className="company-link trading-link">
                View Stock Analysis
              </Link>
            )}
            {ipo.status === 'TRADING' && (ipo.ticker_final || ipo.ticker_proposed) && !ipo.company_id && (
              <span className="company-link trading-link disabled" title="Company data not yet available">
